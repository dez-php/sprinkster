...
class Acl
{
    ...
    /**
     * @var \Core\Acl\Role\RoleInterface
     */
    protected $_isAllowedRole     = null;

    /**
     * @var \Core\Acl\Resource\ResourceInterface
     */
     ...
     * @uses   \Core\Acl\Role\Registry::add()
     * @return \Core\Acl Provides a fluent interface
     */
    public function addRole($role, $parents = null)
    {
        if (is_string($role)) {
            $role = new \Core\Acl\Role($role);
        }

        if (!$role instanceof \Core\Acl\Role\RoleInterface) {
            require_once 'Core/Acl/Exception.php';
            throw new \Core\Acl\Exception('addRole() expects $role to be of type \Core\Acl\Role\RoleInterface');
        }
     ...
     *
     * @param  \Core\Acl\Role\RoleInterface|string $role
     * @uses   \Core\Acl\Role\Registry::has()
     * @return boolean
     */
     ...
     * @uses   \Core\Acl\Role\Registry::inherits()
     * @return boolean
     */
     ...
     *
     * @param  \Core\Acl\Role\RoleInterface|string $role
     * @uses   \Core\Acl\Role\Registry::remove()
     * @return \Core\Acl Provides a fluent interface
     */
     ...
     *
     * @uses   \Core\Acl\Role\Registry::removeAll()
     * @return \Core\Acl Provides a fluent interface
     */
     ...
     *
     * @param  \Core\Acl\Resource\ResourceInterface|string $resource
     * @param  \Core\Acl\Resource\ResourceInterface|string $parent
     * @throws \Core\Acl\Exception
     * @return \Core\Acl Provides a fluent interface
     */
    public function addResource($resource, $parent = null)
    {
        if (is_string($resource)) {
            $resource = new \Core\Acl\Resource($resource);
        }

        if (!$resource instanceof \Core\Acl\Resource\ResourceInterface) {
            require_once 'Core/Acl/Exception.php';
            throw new \Core\Acl\Exception('addResource() expects $resource to be of type \Core\Acl\Resource\ResourceInterface');
        }
        ...
        if ($this->has($resourceId)) {
            require_once 'Core/Acl/Exception.php';
            throw new \Core\Acl\Exception("Resource id '$resourceId' already exists in the ACL");
        }
        ...
        if (null !== $parent) {
            try {
                if ($parent instanceof \Core\Acl\Resource\ResourceInterface) {
                    $resourceParentId = $parent->getResourceId();
                } else {
                    $resourceParentId = $parent;
                }
                $resourceParent = $this->get($resourceParentId);
            } catch (\Core\Acl\Exception $e) {
                require_once 'Core/Acl/Exception.php';
                throw new \Core\Acl\Exception("Parent Resource id '$resourceParentId' does not exist", 0, $e);
            }
     ...
     * @return \Core\Acl Provides a fluent interface
     */
    public function add(\Core\Acl\Resource\ResourceInterface $resource, $parent = null)
    {
     ...
     * @param  \Core\Acl\Resource\ResourceInterface|string $resource
     * @throws \Core\Acl\Exception
     * @return \Core\Acl\Resource\ResourceInterface
     */
    public function get($resource)
    {
        ...
        if (!$this->has($resource)) {
            require_once 'Core/Acl/Exception.php';
            throw new \Core\Acl\Exception("Resource '$resourceId' not found");
        }
     ...
     * @return boolean
     */
    public function hasRegExp($resource) {
    	if ($resource instanceof \Core\Acl\Resource\ResourceInterface) {
    		$resourceId = $resource->getResourceId();
    	} else {
    		$resourceId = (string) $resource;
    	...
    	foreach($this->_resources AS $resourceKey => $resourceData) {
    		if(preg_match('~' . $resourceId . '~i', $resourceKey, $match)) {
    			return true;
    		}
    	}
    	return false;
     ...
     *
     * @param  \Core\Acl\Resource\ResourceInterface|string $resource
     * @param  \Core\Acl\Resource\ResourceInterface|string $inherit
     * @param  boolean                            $onlyParent
     * @throws \Core\Acl\Role\Registry_Registry_Exception
     * @return boolean
     */
    public function inherits($resource, $inherit, $onlyParent = false)
    {
        try {
            ...
            require_once 'Core/Acl/Exception.php';
            throw new \Core\Acl\Exception($e->getMessage(), $e->getCode(), $e);
        }
     ...
     *
     * @param  \Core\Acl\Role\RoleInterface|string|array     $roles
     * @param  \Core\Acl\Resource\ResourceInterface|string|array $resources
     * @param  string|array                             $privileges
     * @param  \Core\Acl\Assert\AssertInterface                $assert
     * @uses   \Core\Acl::setRule()
     * @return \Core\Acl Provides a fluent interface
     */
    public function allow($roles = null, $resources = null, $privileges = null, \Core\Acl\Assert\AssertInterface $assert = null)
    {
     ...
     * @return \Core\Acl Provides a fluent interface
     */
    public function deny($roles = null, $resources = null, $privileges = null, \Core\Acl\Assert\AssertInterface $assert = null)
    {
    ...
    public function setRule($operation, $type, $roles = null, $resources = null, $privileges = null,
                            \Core\Acl\Assert\AssertInterface $assert = null)
    {
        // ensure that the rule type is valid; normalize input to uppercase
        $type = strtoupper($type);
        if (self::TYPE_ALLOW !== $type && self::TYPE_DENY !== $type) {
            require_once 'Core/Acl/Exception.php';
            throw new \Core\Acl\Exception("Unsupported rule type; must be either '" . self::TYPE_ALLOW . "' or '"
                                       . self::TYPE_DENY . "'");
        }
        ...
        switch ($operation) {
            ...
            default:
                require_once 'Core/Acl/Exception.php';
                throw new \Core\Acl\Exception("Unsupported operation; must be either '" . self::OP_ADD . "' or '"
                                           . self::OP_REMOVE . "'");
        }
     ...
     * (allow everything to all). By default, \Core\Acl creates a "whitelist" rule (deny
    ...
    public function isAllowed($role = null, $resource = null, $privilege = null)
    {
        ...
        if (null !== $role) {
            ...
            if (!$this->_isAllowedRole instanceof \Core\Acl\Role\RoleInterface) {
                $this->_isAllowedRole = $role;
            }
        }
        ...
        if (null !== $resource) {
            ...
            if (!$this->_isAllowedResource instanceof \Core\Acl\Resource\ResourceInterface) {
                $this->_isAllowedResource = $resource;
            }
        }
     ...
     * @return boolean
     */
    public function isAllowedRegExp($role = null, $resource = null, $privilege = null) {
    	if ($resource instanceof \Core\Acl\Resource\ResourceInterface) {
    		$resourceId = $resource->getResourceId();
    	} else {
    		$resourceId = (string) $resource;
    	...
    	foreach($this->_resources AS $resourceKey => $resourceData) {
    		if(preg_match('~' . $resourceId . '~i', $resourceKey, $match)) {
    			return $this->isAllowed($role, $resourceKey, $privilege);
    		}
    	}
    	return false;
    }
     ...
     *
     * @return \Core\Acl\Role\Registry
     */
    protected function _getRoleRegistry()
    {
        if (null === $this->_roleRegistry) {
            $this->_roleRegistry = new \Core\Acl\Role\Registry();
        }
     ...
     * @return boolean|null
     */
    protected function _roleDFSAllPrivileges(\Core\Acl\Role\RoleInterface $role, \Core\Acl\Resource\ResourceInterface $resource = null)
    {
        $dfs = array(
            'visited' => array(),
            'stack'   => array()
     ...
     * @throws \Core\Acl\Exception
     */
    protected function _roleDFSVisitAllPrivileges(\Core\Acl\Role\RoleInterface $role, \Core\Acl\Resource\ResourceInterface $resource = null,
                                                 &$dfs = null)
    {
        if (null === $dfs) {
            /**
             * @see \Core\Acl\Exception
             */
            require_once 'Core/Acl/Exception.php';
            throw new \Core\Acl\Exception('$dfs parameter may not be null');
        }
     ...
     * @throws \Core\Acl\Exception
     */
    protected function _roleDFSOnePrivilege(\Core\Acl\Role\RoleInterface $role, \Core\Acl\Resource\ResourceInterface $resource = null,
                                            $privilege = null)
    {
        if (null === $privilege) {
            /**
             ...
             */
            require_once 'Core/Acl/Exception.php';
            throw new \Core\Acl\Exception('$privilege parameter may not be null');
        }
     ...
     * @throws \Core\Acl\Exception
     */
    protected function _roleDFSVisitOnePrivilege(\Core\Acl\Role\RoleInterface $role, \Core\Acl\Resource\ResourceInterface $resource = null,
     ...
     * @return string|null
     */
    protected function _getRuleType(\Core\Acl\Resource\ResourceInterface $resource = null, \Core\Acl\Role\RoleInterface $role = null,
                                    $privilege = null)
    {
        ...
        if ($rule['assert']) {
            ...
            $assertionValue = $assertion->assert(
                $this,
                ($this->_isAllowedRole instanceof \Core\Acl\Role\RoleInterface) ? $this->_isAllowedRole : $role,
                ($this->_isAllowedResource instanceof \Core\Acl\Resource\ResourceInterface) ? $this->_isAllowedResource : $resource,
     ...
     * @return array|null
     */
    protected function &_getRules(\Core\Acl\Resource\ResourceInterface $resource = null, \Core\Acl\Role\RoleInterface $role = null,
                                  $create = false)
    {
    ...
    {
        return count($this->_resources) > 0 ? true : false;
    }
