...
class AssetManager {
	 ...
	 */
	const DEFAULT_BASEPATH='cache/assets';
	/**
	...
	*/
	public function getBasePath()
	{
		...
		{
			$this->setBasePath(rtrim(\Core\Base\Init::getBase(),'\\/').DIRECTORY_SEPARATOR.self::DEFAULT_BASEPATH);
		}
	...
	public function setBasePath($value)
	{
		...
		if(($basePath=realpath($value))!==false && is_dir($basePath) && is_writable($basePath)) {
			...
			throw new \Core\Exception('\Core\AssetManager.basePath "'.$value.'" is invalid. Please make sure the directory exists and is writable by the Web server process.');
		}
	}
	...
	public function getBaseUrl()
	{
		if($this->_baseUrl===null)
		{
			...
			$this->setBaseUrl(rtrim($request->getBaseUrl(),'/').'/'.self::DEFAULT_BASEPATH);
		}
	...
	public function publish($path,$hashByName=false,$level=-1,$forceCopy=null)
	{
		...
		if($forceCopy && $this->linkAssets) {
			throw new \Core\Exception('The "forceCopy" and "linkAssets" cannot be both true.');
		}
		if(isset($this->_published[$path])) {
			...
			elseif(is_dir($src))
			{
				...
				elseif(!is_dir($dstDir) || $forceCopy || @$this->mostRecentModifiedFileTime($dstDir)<@$this->mostRecentModifiedFileTime($src))
				{
					\Core\Utils\FileHelper::copyDirectory($src,$dstDir,array(
		...
		throw new \Core\Exception('The asset "'.$path.'" to be published does not exist.');
	}
	
	public function mostRecentModifiedFileTime($dirName,$doRecursive = true) {
		$d = dir($dirName);
		$lastModified = 0;
		while($entry = $d->read()) {
			if ($entry != "." && $entry != "..") {
				if (!is_dir($dirName."/".$entry)) {
					...
					$currentModified = $this->mostRecentModifiedFileTime($dirName."/".$entry,true);
				}
	...
	{
		return sprintf('%x',crc32($path.\Core\Base\Init::VERSION));
	}
