...
class Apc extends \Core\Cache\Backend implements \Core\Cache\Backend\ExtendedInterface {
	/**
	 * Log message
	 */
	const TAGS_UNSUPPORTED_BY_CLEAN_OF_APC_BACKEND = '\Core\Cache\Backend\Apc::clean() : tags are unsupported by the Apc backend';
	const TAGS_UNSUPPORTED_BY_SAVE_OF_APC_BACKEND = '\Core\Cache\Backend\Apc::save() : tags are unsupported by the Apc backend';
	...
	public function __construct(array $options = array()) {
		if (! extension_loaded ( 'apc' )) {
			\Core\Cache\Base::throwException ( 'The apc extension must be loaded for using this backend !' );
		}
	...
	public function clean($mode = \Core\Cache\Base::CLEANING_MODE_ALL, $tags = array()) {
		switch ($mode) {
			...
			case \Core\Cache\Base::CLEANING_MODE_OLD :
				// $this->_log("\Core\Cache\Backend\Apc::clean() :
	...
	public function getFillingPercentage() {
		...
		if ($memSize == 0) {
			\Core\Cache\Base::throwException ( 'can\'t get apc memory size' );
		}
		if ($memUsed > $memSize) {
			return 100;
		}
