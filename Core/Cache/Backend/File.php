...
class File extends \Core\Cache\Backend implements \Core\Cache\Backend\ExtendedInterface {
	/**
	 * Available options
	 *
	 * =====> (string) cache_dir :
	...
	public function __construct(array $options = array()) {
		...
		if (isset ( $this->_options ['file_name_prefix'] )) { // particular case for
			...
			if (! preg_match ( '~^[a-zA-Z0-9_]+$~D', $this->_options ['file_name_prefix'] )) {
				\Core\Cache\Base::throwException ( 'Invalid file_name_prefix : must use only [a-zA-Z0-9_]' );
			}
		}
		if ($this->_options ['metadatas_array_max_size'] < 10) {
			\Core\Cache\Base::throwException ( 'Invalid metadatas_array_max_size, must be > 10' );
		}
	...
	public function setCacheDir($value, $trailingSeparator = true) {
		if (! is_dir ( $value )) {
			\Core\Cache\Base::throwException ( sprintf ( 'cache_dir "%s" must be a directory', $value ) );
		}
		if (! is_writable ( $value )) {
			\Core\Cache\Base::throwException ( sprintf ( 'cache_dir "%s" is not writable', $value ) );
		}
	...
	public function load($id, $doNotTestCacheValidity = false) {
		...
		if ($this->_options ['read_control']) {
			...
			if ($hashData != $hashControl) {
				...
				// $this->_log('\Core\Cache\Backend\File::load() / read_control
				// : stored hash and computed hash do not match');
				$this->remove ( $id );
				return false;
			}
		}
	...
	public function save($data, $id, $tags = array(), $specificLifetime = false) {
		...
		if (! $res) {
			// $this->_log('\Core\Cache\Backend\File::save() / error on saving
			// metadata');
			return false;
		}
	...
	public function getFillingPercentage() {
		...
		if ($total == 0) {
			\Core\Cache\Base::throwException ( 'can\'t get disk_total_space' );
		} else {
			if ($free >= $total) {
				return 100;
			}
	...
	protected function _remove($file) {
		...
		if (! @unlink ( $file )) {
			...
			// $this->_log("\Core\Cache\Backend\File::_remove() : we can't
			// remove $file");
			return false;
		}
		return true;
	 ...
	 * @return boolean True if no problem
	 */
	protected function _clean($dir, $mode = \Core\Cache\Base::CLEANING_MODE_ALL, $tags = array()) {
		if (! is_dir ( $dir )) {
			return false;
		}
		$result = true;
		...
		foreach ( $glob as $file ) {
			if (is_file ( $file )) {
				...
				if ($this->_isMetadatasFile ( $fileName )) {
					...
					// old metadatas files
					if ($mode != \Core\Cache\Base::CLEANING_MODE_ALL) {
						continue;
					}
				}
				$id = $this->_fileNameToId ( $fileName );
			...
			if (is_file ( $file )) {
				...
				switch ($mode) {
					...
					case 'matchingAny' :
						...
						break;
					default :
						\Core\Cache\Base::throwException ( 'Invalid mode for _get() method' );
						break;
				}
			}
			if ((is_dir ( $file )) and ($this->_options ['hashed_directory_level'] > 0)) {
				...
				if ($recursiveRs === false) {
					// $this->_log('\Core\Cache\Backend\File::_get() / recursive
				// call : can\'t list entries of "'.$file.'"');
				} else {
	...
	protected function _hash($data, $controlType) {
		switch ($controlType) {
			...
			default :
				\Core\Cache\Base::throwException ( "Incorrect hash function : $controlType" );
		}
	}
	...
	protected function _recursiveMkdirAndChmod($id) {
		...
		foreach ( $partsArray as $part ) {
			if (! is_dir ( $part )) {
				...
				@chmod ( $part, $this->_options ['hashed_directory_perm'] ); // see
				                                                         // #ZF-320 (this
				                                                         // line is required
				                                                         // in some
				                                                         // configurations)
	 ...
	 * @param boolean $doNotTestCacheValidity
	 *        	If set to true, the cache validity won't be tested
	 * @return boolean mixed (a cache is not available) or "last modified"
	 *         timestamp (int) of the available cache record
	 */
