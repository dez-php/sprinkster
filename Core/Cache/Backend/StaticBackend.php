...
class StaticBackend extends \Core\Cache\Backend implements \Core\Cache\Backend\InterfaceBackend {
	const INNER_CACHE_NAME = '\Core\Cache\Backend\StaticBackend\Tagcache';
	 ...
	 * @param string $name        	
	 * @param mixed $value        	
	 * @return \Core\Cache\Backend\StaticBackend
	 */
	...
	public function load($id, $doNotTestCacheValidity = false) {
		...
		if (! $this->_verifyPath ( $id )) {
			\Core\Cache\Base::throwException ( 'Invalid cache id: does not match expected public_dir path' );
		}
		if ($doNotTestCacheValidity) {
			// $this->_log("\Core\Cache\Backend\StaticBackend::load() :
		// \$doNotTestCacheValidity=true is unsupported by the Static backend");
		}
	...
	protected function _createDirectoriesFor($path) {
		if (! is_dir ( $path )) {
			...
			if (! @mkdir ( $path, $this->_octdec ( $this->_options ['cache_directory_umask'] ), true )) {
				...
				umask ( $oldUmask );
				\Core\Cache\Base::throwException ( "Can't create directory: {$lastErr['message']}" );
			}
			umask ( $oldUmask );
		}
	}
	...
	public function clean($mode = \Core\Cache\Base::CLEANING_MODE_ALL, $tags = array()) {
		...
		switch ($mode) {
			...
			case \Core\Cache\Base::CLEANING_MODE_MATCHING_ANY_TAG :
				if (empty ( $tags )) {
					throw new \Core\Exception ( 'Cannot use tag matching modes as no tags were defined' );
				}
				if ($this->_tagged === null && $tagged = $this->getInnerCache ()->load ( self::INNER_CACHE_NAME )) {
					$this->_tagged = $tagged;
				} elseif (! $this->_tagged) {
			...
			case \Core\Cache\Base::CLEANING_MODE_OLD :
				// $this->_log("\Core\Cache\Backend\StaticBackend : Selected
				// Cleaning Mode Currently Unsupported By This Backend");
				break;
	 ...
	 * @return void
	 */
	public function setInnerCache(\Core\Cache\Core $cache) {
	...
	public function getInnerCache() {
		if ($this->_tagCache === null) {
			\Core\Cache\Base::throwException ( 'An Inner Cache has not been set; use setInnerCache()' );
		}
	...
	protected static function _validateIdOrTag($string) {
		...
		if (! preg_match ( '/^(?:\/(?:(?:%[[:xdigit:]]{2}|[A-Za-z0-9-_.!~*\'()\[\]:@&=+$,;])*)?)+$/', $string )) {
			\Core\Cache\Base::throwException ( "Invalid id or tag '$string' : must be a valid URL path" );
		}
	}
