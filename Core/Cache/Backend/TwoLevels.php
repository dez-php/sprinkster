...
class TwoLevels extends \Core\Cache\Backend implements \Core\Cache\Backend\ExtendedInterface {
	/**
	 * Available options
	 *
	 * =====> (string) slow_backend :
	 * - Slow backend name
	 * - Must implement the \Core\Cache\Backend\ExtendedInterface
	 ...
	 * - See \Core\Cache\Base::factory() method
	 *
	 ...
	 * Slow Backend
	 *
	 * @var \Core\Cache\Backend\ExtendedInterface
	 */
	...
	public function __construct(array $options = array()) {
		...
		if ($this->_options ['slow_backend'] === null) {
			\Core\Cache\Base::throwException ( 'slow_backend option has to set' );
		} elseif ($this->_options ['slow_backend'] instanceof \Core\Cache\Backend\ExtendedInterface) {
			$this->_slowBackend = $this->_options ['slow_backend'];
		} else {
			$this->_slowBackend = \Core\Cache\Base::_makeBackend ( $this->_options ['slow_backend'], $this->_options ['slow_backend_options'], $this->_options ['slow_backend_custom_naming'], $this->_options ['slow_backend_autoload'] );
			if (! in_array ( '\Core\Cache\Backend\ExtendedInterface', class_implements ( $this->_slowBackend ) )) {
				\Core\Cache\Base::throwException ( 'slow_backend must implement the \Core\Cache\Backend\ExtendedInterface interface' );
			}
		}
		
		if ($this->_options ['fast_backend'] === null) {
			\Core\Cache\Base::throwException ( 'fast_backend option has to set' );
		} elseif ($this->_options ['fast_backend'] instanceof \Core\Cache\Backend\ExtendedInterface) {
			$this->_fastBackend = $this->_options ['fast_backend'];
		} else {
			$this->_fastBackend = \Core\Cache\Base::_makeBackend ( $this->_options ['fast_backend'], $this->_options ['fast_backend_options'], $this->_options ['fast_backend_custom_naming'], $this->_options ['fast_backend_autoload'] );
			if (! in_array ( '\Core\Cache\Backend\ExtendedInterface', class_implements ( $this->_fastBackend ) )) {
				\Core\Cache\Base::throwException ( 'fast_backend must implement the \Core\Cache\Backend\ExtendedInterface interface' );
			}
		}
	 ...
	 *
	 * Available modes are :
	 * \Core\Cache\Base::CLEANING_MODE_ALL (default) => remove all cache entries
	 * ($tags is not used)
	 * \Core\Cache\Base::CLEANING_MODE_OLD => remove too old cache entries
	 * ($tags is not used)
	 * \Core\Cache\Base::CLEANING_MODE_MATCHING_TAG => remove cache entries
	 * matching all given tags
	 * ($tags can be an array of strings or a single string)
	 * \Core\Cache\Base::CLEANING_MODE_NOT_MATCHING_TAG => remove cache entries
	 * not {matching one of the given tags}
	 * ($tags can be an array of strings or a single string)
	 * \Core\Cache\Base::CLEANING_MODE_MATCHING_ANY_TAG => remove cache entries
	...
	public function clean($mode = \Core\Cache\Base::CLEANING_MODE_ALL, $tags = array()) {
		switch ($mode) {
			case \Core\Cache\Base::CLEANING_MODE_ALL :
				$boolFast = $this->_fastBackend->clean ( \Core\Cache\Base::CLEANING_MODE_ALL );
				$boolSlow = $this->_slowBackend->clean ( \Core\Cache\Base::CLEANING_MODE_ALL );
				return $boolFast && $boolSlow;
				break;
			case \Core\Cache\Base::CLEANING_MODE_OLD :
				return $this->_slowBackend->clean ( \Core\Cache\Base::CLEANING_MODE_OLD );
