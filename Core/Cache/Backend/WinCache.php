...
class WinCache extends \Core\Cache\Backend implements \Core\Cache\Backend\ExtendedInterface {
	/**
	 * Log message
	 */
	const TAGS_UNSUPPORTED_BY_CLEAN_OF_WINCACHE_BACKEND = '\Core\Cache\Backend\WinCache::clean() : tags are unsupported by the WinCache backend';
	const TAGS_UNSUPPORTED_BY_SAVE_OF_WINCACHE_BACKEND = '\Core\Cache\Backend\WinCache::save() : tags are unsupported by the WinCache backend';
	...
	public function __construct(array $options = array()) {
		if (! extension_loaded ( 'wincache' )) {
			\Core\Cache\Base::throwException ( 'The wincache extension must be loaded for using this backend !' );
		}
	...
	public function clean($mode = \Core\Cache\Base::CLEANING_MODE_ALL, $tags = array()) {
		switch ($mode) {
			...
			case \Core\Cache\Base::CLEANING_MODE_OLD :
				// $this->_log("\Core\Cache\Backend\WinCache::clean() :
				// CLEANING_MODE_OLD is unsupported by the WinCache backend");
				break;
	...
	public function getFillingPercentage() {
		...
		if ($memSize == 0) {
			\Core\Cache\Base::throwException ( 'can\'t get WinCache memory size' );
		}
		if ($memUsed > $memSize) {
			return 100;
		}
