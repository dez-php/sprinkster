...
class ClassFrontend extends \Core\Cache\Core {
	/**
	 * Available options
	 *
	 * ====> (mixed) cached_entity :
	...
	public function __construct(array $options = array()) {
		...
		if ($this->_specificOptions ['cached_entity'] === null) {
			\Core\Cache\Base::throwException ( 'cached_entity must be set !' );
		}
	...
	public function setCachedEntity($cachedEntity) {
		if (! is_string ( $cachedEntity ) && ! is_object ( $cachedEntity )) {
			\Core\Cache\Base::throwException ( 'cached_entity must be an object or a class name' );
		}
