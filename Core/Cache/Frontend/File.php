...
class File extends \Core\Cache\Core {
	 ...
	 * - \Core\Cache\Frontend\File::MODE_AND or
	 * \Core\Cache\Frontend\File::MODE_OR
	...
	public function __construct(array $options = array()) {
		...
		if (! isset ( $this->_specificOptions ['master_files'] )) {
			\Core\Cache\Base::throwException ( 'master_files option must be set' );
		}
	}
	...
	public function setMasterFiles(array $masterFiles) {
		...
		foreach ( $masterFiles as $masterFile ) {
			...
			if (! $this->_specificOptions ['ignore_missing_master_files'] && ! $mtime) {
				\Core\Cache\Base::throwException ( 'Unable to read master_file : ' . $masterFile );
			}
