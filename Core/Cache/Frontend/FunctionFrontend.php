...
class FunctionFrontend extends \Core\Cache\Core {
	/**
	...
	public function call($callback, array $parameters = array(), $tags = array(), $specificLifetime = false, $priority = 8) {
		if (! is_callable ( $callback, true, $name )) {
			\Core\Cache\Base::throwException ( 'Invalid callback' );
		}
	...
	public function makeId($callback, array $args = array()) {
		...
		if (isset ( $object )) {
			try {
				...
				\Core\Cache\Base::throwException ( $e->getMessage () );
			}
			if (! $tmp) {
				$lastErr = error_get_last ();
				\Core\Cache\Base::throwException ( "Can't serialize callback object to generate id: {$lastErr['message']}" );
			}
			$name .= '__' . $tmp;
		}
		...
		if ($args) {
			...
			if (! $argsStr) {
				$lastErr = error_get_last ();
				throw \Core\Cache\Base::throwException ( "Can't serialize arguments to generate id: {$lastErr['message']}" );
			}
		}
