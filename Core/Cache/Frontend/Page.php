...
class Page extends \Core\Cache\Core {
	/**
	...
	public function __construct(array $options = array()) {
		...
		if (isset ( $this->_specificOptions ['http_conditional'] )) {
			if ($this->_specificOptions ['http_conditional']) {
				\Core\Cache\Base::throwException ( 'http_conditional is not implemented for the moment !' );
			}
		}
	...
	protected function _setRegexps($regexps) {
		if (! is_array ( $regexps )) {
			\Core\Cache\Base::throwException ( 'regexps option must be an array !' );
		}
		foreach ( $regexps as $regexp => $conf ) {
			if (! is_array ( $conf )) {
				\Core\Cache\Base::throwException ( 'regexps option must be an array of arrays !' );
			}
			$validKeys = array_keys ( $this->_specificOptions ['default_options'] );
			foreach ( $conf as $key => $value ) {
				if (! is_string ( $key )) {
					\Core\Cache\Base::throwException ( "unknown option [$key] !" );
				}
				$key = strtolower ( $key );
