...
class Component {
	...
	public function __get($name)
	{
		...
		throw new \Core\Exception('Property "'.get_class($this).'.'.$name.'" is not defined.');
	}
	...
	public function __set($name,$value)
	{
		...
		if(method_exists($this,'get'.$name)) {
			throw new \Core\Exception('Property "'.get_class($this).'.'.$name.'" is read only.');
		} else {
			throw new \Core\Exception('Property "'.get_class($this).'.'.$name.'" is not defined.');
		}
	}
	...
	public function __call($name,$parameters)
	{
		...
		throw new \Core\Exception(get_class($this).' and its behaviors do not have a method or closure named "'.$name.'".');
	}
	 ...
	 * @return \Core\Behavior\BehaviorInterface the behavior object, or null if the behavior does not exist
	 */
	 ...
	 * {@link \Core\Behavior\BehaviorInterface}, a string specifying the behavior class, or an
	 ...
	 * by calling its {@link \Core\Behavior\BehaviorInterface::attach} method.
	 ...
	 * @return \Core\Behavior\BehaviorInterface the behavior object
	 */
	public function attachBehavior($name,$behavior)
	{
		if(!($behavior instanceof \Core\Behavior\BehaviorInterface)) {
			$behavior=\Core\Registry\Component::create($behavior);
		}
	 ...
	 * The behavior's {@link \Core\Behavior\BehaviorInterface::detach} method will be invoked.
	 * @param string $name the behavior's name. It uniquely identifies the behavior.
	 * @return \Core\Behavior\BehaviorInterface the detached behavior. Null if the behavior does not exist.
	 */
	...
	public function getEventHandlers($name)
	{
		if($this->hasEvent($name))
		{
			...
			throw new \Core\Exception('Event "'.get_class($this).'.'.$name.'" is not defined.');
		}
	}
	 ...
	 * @param string $name the event name
	 * @param \Core\Db\Event $event the event parameter
	...
	public function raiseEvent($name,$event)
	{
				...
				if(is_string($handler)) {
					...
					if(is_array($handler)) {
						...
						$object->$method($event);
							...
							throw new \Core\Exception('Event "'.get_class($this).'.'.$name.'" is attached with an invalid handler "'.$handler[1].'".');
					} else { // PHP 5.3: anonymous function
						call_user_func($handler,$event);
					}
				} else {
					throw new \Core\Exception('Event "'.get_class($this).'.'.$name.'" is attached with an invalid handler "'.gettype($handler).'".');
				}
		...
		} elseif(\Core\Base\Action::getInstance()->getApplication()->getEnvironment() != 'production' && !$this->hasEvent($name)) {
			throw new \Core\Exception('Event "'.get_class($this).'.'.$name.'" is not defined.');
		}
	}
