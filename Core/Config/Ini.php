...
class Ini extends \Core\Config\Main {
	/**
	 ...
	 * hostname = staging
	 *
	 * after calling $data = new \Core\Config\Ini($file, 'staging'); then
		...
		if (null === $section) {
			...
			foreach ( $section as $sectionName ) {
				...
				if (isset ( $phpArray [$sectionName] [';extends'] ) && isset ( $phpArray [$phpArray [$sectionName] [';extends']] )) {
					$dataArray = $this->_arrayMergeRecursive ( $this->_arrayMergeRecursive ( $iniArray [$iniArray [$sectionName] [';extends']], $iniArray [$sectionName] ), $dataArray );
				}
		...
		foreach ( $loaded as $key => $data ) {
			...
			switch (count ( $pieces )) {
				...
				case 2 :
					...
					$iniArray [$thisSection] = \Core\Arrays::array_merge ( array (
