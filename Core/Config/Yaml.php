...
class Yaml extends \Core\Config\Main {
	/**
	 ...
	 *        	the decoder to set
	 * @return JO_Config_Yaml
	 */
	public function setYamlDecoder($yamlDecoder) {
		if (! is_callable ( $yamlDecoder )) {
			throw new \Core\Exception ( 'Invalid parameter to setYamlDecoder() - must be callable' );
		}
	...
	public function __construct($yaml, $section = null, $options = false) {
		...
		if (null === $config) {
			// decode failed
			throw new \Core\Exception ( "Error parsing YAML data" );
		}
		...
		if (null === $section) {
			...
			foreach ( $section as $sectionName ) {
				if (! isset ( $config [$sectionName] )) {
					throw new \Core\Exception ( sprintf ( 'Section "%s" cannot be found', implode ( ' ', ( array ) $section ) ) );
				}
				
				$dataArray = \Core\Arrays::array_merge ( $this->_processExtends ( $config, $sectionName ), $dataArray );
			}
			parent::__construct ( $dataArray, $allowModifications );
		} else {
	 ...
	 * @return array
	 * @throws JO_Config_Exception When $section cannot be found
	 */
	protected function _processExtends(array $data, $section, array $config = array()) {
		if (! isset ( $data [$section] )) {
			throw new \Core\Exception ( sprintf ( 'Section "%s" cannot be found', $section ) );
		}
	...
	protected static function _decodeYaml($currentIndent, &$lines) {
		...
		while ( list ( $n, $line ) = each ( $lines ) ) {
			...
			if (preg_match ( "/(?!-)([\w\-]+):\s*(.*)/", $line, $m )) {
				...
				throw new \Core\Exception ( sprintf ( 'Error parsing YAML at line %d - unsupported syntax: "%s"', $lineno, $line ) );
			}
		}
		return $config;
	}
