...
class Sqlsrv extends \Core\Db\Adapter\AbstractAdapter {
	/**
	...
	protected $_numericDataTypes = array (
			...
			'MONEY' => \Core\Db\Init::FLOAT_TYPE,
			'NUMERIC' => \Core\Db\Init::FLOAT_TYPE,
			'REAL' => \Core\Db\Init::FLOAT_TYPE,
			'SMALLMONEY' => \Core\Db\Init::FLOAT_TYPE 
	);
	 ...
	 * @var string
	 */
	protected $_defaultStmtClass = '\Core\Db\Statement\Sqlsrv';
	
	/**
	 * Creates a connection resource.
	 *
	 * @return void
	 * @throws \Core\Db\Adapter\Sqlsrv\Exception
	 */
	protected function _connect() {
		...
		if (! extension_loaded ( 'sqlsrv' )) {
			/**
			 *
			 * @see \Core\Db\Adapter\Sqlsrv\Exception
			 */
			require_once 'Db/Adapter/Sqlsrv/Exception.php';
			throw new \Core\Db\Adapter\Sqlsrv\Exception ( 'The Sqlsrv extension is required for this adapter but the extension is not loaded' );
		}
		...
		if (! $this->_connection) {
			 ...
			 */
			require_once 'Db/Adapter/Sqlsrv/Exception.php';
			throw new \Core\Db\Adapter\Sqlsrv\Exception ( sqlsrv_errors () );
		}
	}
	...
	protected function _checkRequiredOptions(array $config) {
		...
		if (! array_key_exists ( 'password', $config ) && array_key_exists ( 'username', $config )) {
			 ...
			 */
			require_once 'Db/Adapter/Exception.php';
			throw new \Core\Db\Adapter\Exception ( "Configuration array must have a key for 'password' for login credentials.
		...
		if (array_key_exists ( 'password', $config ) && ! array_key_exists ( 'username', $config )) {
			 ...
			 */
			require_once 'Db/Adapter/Exception.php';
			throw new \Core\Db\Adapter\Exception ( "Configuration array must have a key for 'username' for login credentials.
                                                If Windows Authentication is desired, both keys 'username' and 'password' should be ommited from config." );
		}
	}
	...
	public function setTransactionIsolationLevel($level = null) {
		...
		switch ($level) {
			...
			default :
				require_once 'Db/Adapter/Sqlsrv/Exception.php';
				throw new \Core\Db\Adapter\Sqlsrv\Exception ( "Invalid transaction isolation level mode '$level' specified" );
		}
		
		if (! sqlsrv_query ( $this->_connection, "SET TRANSACTION ISOLATION LEVEL $sql;" )) {
			require_once 'Db/Adapter/Sqlsrv/Exception.php';
			throw new \Core\Db\Adapter\Sqlsrv\Exception ( "Transaction cannot be changed to '$level'" );
		}
	 ...
	 * @return \Core\Db\Statement\Sqlsrv
	 */
	...
	public function describeTable($tableName, $schemaName = null) {
		...
		$primaryKeysResult = $stmt->fetchAll ( \Core\Db\Init::FETCH_NUM );
	...
	public function setFetchMode($mode) {
		switch ($mode) {
			...
			case \Core\Db\Init::FETCH_BOUND : // bound to PHP variable
				require_once 'Db/Adapter/Sqlsrv/Exception.php';
				throw new \Core\Db\Adapter\Sqlsrv\Exception ( 'FETCH_BOUND is not supported yet' );
				break;
			default :
				require_once 'Db/Adapter/Sqlsrv/Exception.php';
				throw new \Core\Db\Adapter\Sqlsrv\Exception ( "Invalid fetch mode '$mode' specified" );
				break;
		}
	}
