...
class Profiler {
	...
	/**
	 * Array of \Core\Db\Profiler\Query objects.
	 *
	 * @var array
	 */
	 ...
	 * @return \Core\Db\Profiler Provides a fluent interface
	 */
	 ...
	 * the \Core\Db\Profiler::* constants to only save profiles for that type of
	 ...
	 *
	 * @return integer null
	 * @see \Core\Db\Profiler::setFilterQueryType()
	 */
	 ...
	 */
	public function queryClone(\Core\Db\Profiler\Query $query) {
	...
	/**
	 * Starts a query.
	 * Creates a new query profile object (\Core\Db\Profiler\Query)
	 ...
	 *        	OPTIONAL Type of query, one of the \Core\Db\Profiler::*
	 *        	constants
	 * @return integer null
	 */
	public function queryStart($queryText, $queryType = null) {
		...
		/**
		 *
		 * @see \Core\Db\Profiler\Query
		 */
		require_once 'Db/Profiler/Query.php';
		$this->_queryProfiles [] = new \Core\Db\Profiler\Query ( $queryText, $queryType );
	 ...
	 *
	 * @param integer $queryId        	
	 * @throws \Core\Db\Profiler\Exception
	 * @return void
	 */
	public function queryEnd($queryId) {
		// Don't do anything if the \Core\Db\Profiler is not enabled.
		if (! $this->_enabled) {
			return self::IGNORED;
		}
		...
		if (! isset ( $this->_queryProfiles [$queryId] )) {
			/**
			 *
			 * @see \Core\Db\Profiler_Exception
			 */
			require_once 'Db/Profiler/Exception.php';
			throw new \Core\Db\Profiler\Exception ( "Profiler has no query with handle '$queryId'." );
		}
		...
		if ($qp->hasEnded ()) {
			 ...
			 */
			require_once 'Db/Profiler/Exception.php';
			throw new \Core\Db\Profiler\Exception ( "Query with profiler handle '$queryId' has already ended." );
		}
	 ...
	 * by queryStart() and it will return a \Core\Db\Profiler\Query object.
	 *
	 * @param integer $queryId        	
	 * @throws \Core\Db\Profiler\Exception
	 * @return \Core\Db\Profiler\Query
	 */
	public function getQueryProfile($queryId) {
		if (! array_key_exists ( $queryId, $this->_queryProfiles )) {
			 ...
			 */
			require_once 'Db/Profiler/Exception.php';
			throw new \Core\Db\Profiler\Exception ( "Query handle '$queryId' not found in profiler log." );
		}
	...
	/**
	 * Get an array of query profiles (\Core\Db\Profiler\Query objects).
	 * If $queryType
	 * is set to one of the \Core\Db\Profiler::* constants then only queries of
	 * that
	 ...
	 * one or more of the \Core\Db\Profiler::* constants, the elapsed time will
	 * be calculated
	 ...
	 * be counted. If $queryType is set to one of the \Core\Db\Profiler::*
	...
	/**
	 * Get the \Core\Db\Profiler\Query object for the last query that was run,
	 ...
	 *
	 * @return \Core\Db\Profiler\Query false
	 */
