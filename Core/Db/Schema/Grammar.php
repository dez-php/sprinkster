...
abstract class Grammar {
	 ...
	 * @return void
	 */
	public function __construct(\Core\Db\Adapter\AbstractAdapter $connection) {
	...
	public function wrap_table($table) {
		...
		if ($table instanceof \Core\Db\Expr) {
	...
	public function wrap($value) {
		...
		// so we do not want to wrap them in any way. We will just return
		// the string value from the expression to be included.
		if ($value instanceof \Core\Db\Expr) {
			return $value->get ();
		}
	...
	final public function parameter($value) {
		return ($value instanceof \Core\Db\Expr) ? $value->get () : '?';
	}
