...
abstract class Grammar extends \Core\Db\Schema\Grammar {
	...
	public function foreign(Table $table, Fluent $command) {
		// $name = $command->name;
		$name = $table->name . '_' . $command->on . '_' . $command->references;
		
		// We need to wrap both of the table names in quoted identifiers to
		// protect
	...
	protected function drop_constraint(Table $table, Fluent $command) {
		$name = $table->name . '_' . $command->on . '_' . $command->references;
		return "ALTER TABLE " . $this->wrap ( $table ) . " DROP CONSTRAINT " . $name;
	}
