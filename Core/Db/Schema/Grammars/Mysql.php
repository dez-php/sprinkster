...
namespace Core\Db\Schema\Grammars;

use Core\Db\Schema\Fluent;
use Core\Db\Schema\Table;

class Mysql extends \Core\Db\Schema\Grammars\Grammar {
	...
	public function create(Table $table, Fluent $command) {
		$hasIncrement = false;
		foreach ( $table->columns as $row => $col ) {
			if (isset ( $col->attributes ['increment'] ) && $col->attributes ['increment'] === true) {
				$hasIncrement = $col;
				unset ( $table->columns [$row] );
				break;
			}
		}
		$createTable = new Table ( $table->name );
		if (! $hasIncrement) {
			$createTable->increments ( $table->name . '_id' );
		} else {
			$createTable->increments ( $hasIncrement->attributes ['name'] );
		}
		
		$columns = implode ( ', ', $this->columns ( $createTable ) );
		
		// First we will generate the base table creation statement. Other than
		// auto
		// incrementing keys, no indexes will be created during the first
		// creation
		// of the table as they're added in separate commands.
		$sql = array (
				0 => 'CREATE TABLE IF NOT EXISTS ' . $this->wrap ( $table->name ) . ' (' . $columns . ')' 
		);
		
		if (! is_null ( $table->engine )) {
			$sql [0] .= ' ENGINE = ' . $table->engine;
		}
		
		foreach ( $table->columns as $col ) {
			$tableAdd = new Table ( $table->name );
			$tableAdd->columns [] = $col;
			$sql [] = $this->add ( $tableAdd, $col );
		}
	...
	public function add(Table $table, Fluent $command) {
		...
		// Once we have the array of column definitions, we need to add "add" to
		// the
	...
	protected function nullable(Table $table, Fluent $column) {
		if (! is_null ( $column->defaults )) {
			if (trim ( strtolower ( $column->defaults ) ) == 'null') {
				return ' NULL';
			} else {
