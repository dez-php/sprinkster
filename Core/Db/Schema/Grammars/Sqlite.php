...
class Sqlite extends \Core\Db\Schema\Grammars\Grammar {
	...
	public function create(Table $table, Fluent $command) {
		...
		// incrementing
		// keys, no indexes will be created during the first creation of the
		// table since
		...
		// the creation
		// of the table, so we'll need to sniff out any primary keys here and
		// add them to
		// the table now during this command.
		$primary = \Core\Arrays::array_first ( $table->commands, function ($key, $value) {
			return $value->type == 'primary';
		} );
		
		// If we found primary keys in the array of commands, we'll create the
		// SQL for
		// the key addition and append it to the SQL table creation statement
		// for
