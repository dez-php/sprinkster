...
class Select {
	 ...
	 * Class constructor
	 *
	 * @param \Core\Db\Adapter\Abs $adapter        	
	 */
	public function __construct(\Core\Db\Adapter\AbstractAdapter $adapter) {
	 ...
	 * table.
	 *
	 * The second parameter can be a single string or \Core\Db\Expr object,
	 * or else an array of strings or \Core\Db\Expr objects.
	 *
	 * The first parameter can be null or an empty string, in which case
	 * no correlation name is generated or prepended to the columns named
	 * in the second parameter.
	 *
	 * @param array|string|\Core\Db\Expr $name
	 *        	The table name or an associative array relating table name to
	 *        	correlation name.
	 ...
	 *
	 * The parameter can be a single string or \Core\Db\Expr object,
	 * or else an array of strings or \Core\Db\Expr objects.
	 *
	 * @param array|string|\Core\Db\Expr $cols
	 *        	The columns to select from this table.
	...
	public function columns($cols = '*', $correlationName = null) {
		...
		if (! array_key_exists ( $correlationName, $this->_parts [self::FROM] )) {
			/**
			 *
			 * @see \Core\Db\Select\Exception
			 */
			require_once 'Db/Select/Exception.php';
			throw new \Core\Db\Select\Exception ( "No table has been specified for the FROM clause" );
		}
	 ...
	 *
	 * The first parameter has to be an array of \Core\Db\Select or
	 * sql query strings.
	 *
	 * <code>
	 * $sql1 = $db->select();
	...
	public function union($select = array(), $type = self::SQL_UNION) {
		if (! is_array ( $select )) {
			require_once 'Db/Select/Exception.php';
			throw new \Core\Db\Select\Exception ( "union() only accepts an array of \\Core\\Db\\Select instances of sql query strings." );
		}
		
		if (! in_array ( $type, self::$_unionTypes )) {
			require_once 'Db/Select/Exception.php';
			throw new \Core\Db\Select\Exception ( "Invalid union type '{$type}'" );
		}
	 ...
	 * @param string|\Core\Db\Expr $val
	 *        	The value to quote into the condition.
	 * @return \Core\Db\Select This \Core\Db\Select object.
	 */
	...
	public function order($spec) {
		...
		foreach ( $spec as $val ) {
			if ($val instanceof \Core\Db\Expr) {
				$expr = $val->__toString ();
				if (empty ( $expr )) {
					continue;
				}
	 ...
	 * @param string $part        	
	 * @return mixed
	 * @throws \Core\Db\Select\Exception
	 */
	public function getPart($part) {
		$part = strtolower ( $part );
		if (! $this->issetPart ( $part )) {
			require_once 'Db/Select/Exception.php';
			throw new \Core\Db\Select\Exception ( "Invalid Select part '$part'" );
		}
	...
	public function issetPart($part) {
		return array_key_exists ( strtolower ( $part ), $this->_parts );
	}
	 ...
	 * @return PDO_Statement \Core\Db\Statement
	 */
	...
	/**
	 * Gets the \Core\Db\Adapter\AbstractAdapter for this
	 * particular \Core\Db\Select object.
	 *
	...
	protected function _join($type, $name, $cond, $cols, $schema = null) {
		if (! in_array ( $type, self::$_joinTypes ) && $type != self::FROM) {
			 ...
			 */
			require_once 'Db/Select/Exception.php';
			throw new \Core\Db\Select\Exception ( "Invalid join type '$type'" );
		}
		
		if (count ( $this->_parts [self::UNION] )) {
			require_once 'Db/Select/Exception.php';
			throw new \Core\Db\Select\Exception ( "Invalid use of table with " . self::SQL_UNION );
		}
		...
		} else if ($name instanceof \Core\Db\Expr || $name instanceof \Core\Db\Select) {
			$tableName = $name;
		...
		if (! empty ( $correlationName )) {
			if (array_key_exists ( $correlationName, $this->_parts [self::FROM] )) {
				 ...
				 */
				require_once 'Db/Select/Exception.php';
				throw new \Core\Db\Select\Exception ( "You cannot define a correlation name '$correlationName' more than once" );
			}
	...
	public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null) {
		if (empty ( $this->_parts [self::FROM] )) {
			require_once 'Db/Select/Exception.php';
			throw new \Core\Db\Select\Exception ( "You can only perform a joinUsing after specifying a FROM table" );
		}
	...
	protected function _where($condition, $value = null, $type = null, $bool = true) {
		if (count ( $this->_parts [self::UNION] )) {
			require_once 'Db/Select/Exception.php';
			throw new \Core\Db\Select\Exception ( "Invalid use of where clause with " . self::SQL_UNION );
		}
		
		if ($value !== null) {
			if(is_array($value)) {
				$condition = $this->_whereSprintf($condition, $value, $type);
			} else {
	...
	protected function _whereSprintf($condition='', $value=[], $type = null, $char='%') {
		if (!$condition) return '';
		if (count($value) > 0)
		{
			list($key) = each($value);
			if($key === (int)$key || strpos($condition, $char . $key) === false) 
				return $this->_adapter->quoteInto ( $condition, $value, $type );
			foreach ($value as $k => $v)
			{
				$condition = str_replace($char . $k, $this->_adapter->quote ( $v, $type ), $condition);
			}
		}
	...
	protected function _renderColumns($sql) {
		...
		foreach ( $this->_parts [self::COLUMNS] as $columnEntry ) {
			...
			if ($column instanceof \Core\Db\Expr) {
				$columns [] = $this->_adapter->quoteColumnAs ( $column, $alias, true );
			} else {
				if ($column == self::SQL_WILDCARD) {
					$column = new \Core\Db\Expr ( self::SQL_WILDCARD );
					$alias = null;
				}
				if (empty ( $correlationName )) {
					$columns [] = $this->_adapter->quoteColumnAs ( $column, $alias, true );
	...
	protected function _renderUnion($sql) {
		if ($this->_parts [self::UNION]) {
			...
			foreach ( $this->_parts [self::UNION] as $cnt => $union ) {
				...
				if ($target instanceof \Core\Db\Select) {
	 ...
	 * @return \Core\Db\Select
	 * @throws \Core\Db\Select\Exception If an invalid method is called.
	 */
		...
		if (preg_match ( '/^join([a-zA-Z]*?)Using$/', $method, $matches )) {
			...
			if ($type) {
				...
				if (! in_array ( $type, self::$_joinTypes )) {
					require_once 'Db/Select/Exception.php';
					throw new \Core\Db\Select\Exception ( "Unrecognized method '$method()'" );
				}
				if (in_array ( $type, array (
					...
					require_once 'Db/Select/Exception.php';
					throw new \Core\Db\Select\Exception ( "Cannot perform a joinUsing with method '$method()'" );
				}
			} else {
	...
	public function useIndex($index) {
		if (empty ( $this->_parts [self::FORCE_INDEX] )) {
			...
			if (isset ( $this->_parts [self::USE_INDEX] )) {
				$this->_parts [self::USE_INDEX] = array_merge ( $this->_parts [self::USE_INDEX], $index );
			} else {
				$this->_parts [self::USE_INDEX] = $index;
			}
			return $this;
		} else {
			throw new \Core\Db\Select\Exception ( "Cannot use 'USE INDEX' in the same query as 'FORCE INDEX'" );
		}
	}
	...
	public function forceIndex($index) {
		if (empty ( $this->_parts [self::USE_INDEX] )) {
			...
			throw new \Core\Db\Select\Exception ( "Cannot use 'FORCE INDEX' in the same query as 'USE INDEX'" );
		}
	}
