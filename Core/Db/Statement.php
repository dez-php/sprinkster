...
abstract class Statement implements \Core\Db\Statement\InterfaceStatement {
	 ...
	 * @var integer
	 */
	protected $_fetchMode = \Core\Db\Init::FETCH_ASSOC;
	 ...
	 *
	 * @param \Core\Db\Init_Adapter_Abstract $adapter        	
	 * @param mixed $sql
	 *        	Either a string or \Core\Db\Select.
	 */
	public function __construct($adapter, $sql) {
		$this->_adapter = $adapter;
		if ($sql instanceof \Core\Db\Select) {
			$sql = $sql->assemble ();
		}
		...
		foreach ( $this->_sqlSplit as $key => $val ) {
			if ($val == '?') {
				if ($this->_adapter->supportsParameters ( 'positional' ) === false) {
					/**
					 *
					 * @see \Core\Db\Init_Statement_Exception
					 */
					require_once 'Db/Statement/Exception.php';
					throw new \Core\Db\Statement\Exception ( "Invalid bind-variable position '$val'" );
				}
			} else if ($val [0] == ':') {
				if ($this->_adapter->supportsParameters ( 'named' ) === false) {
					 ...
					 */
					require_once 'Db/Statement/Exception.php';
					throw new \Core\Db\Statement\Exception ( "Invalid bind-variable name '$val'" );
				}
			}
			$this->_sqlParam [] = $val;
		}
	...
	public function bindParam($parameter, &$variable, $type = null, $length = null, $options = null) {
		if (! is_int ( $parameter ) && ! is_string ( $parameter )) {
			/**
			 *
			 * @see \Core\Db\Statement\Exception
			 */
			require_once 'Db/Statement/Exception.php';
			throw new \Core\Db\Statement\Exception ( 'Invalid bind-variable position' );
		}
		...
		if ($position === null) {
			 ...
			 */
			require_once 'Db/Statement/Exception.php';
			throw new \Core\Db\Statement\Exception ( "Invalid bind-variable position '$parameter'" );
		}
	...
	public function fetchAll($style = null, $col = null) {
		$data = array ();
		if ($style === \Core\Db\Init::FETCH_COLUMN && $col === null) {
			$col = 0;
		}
		if ($col === null) {
	...
	public function fetchColumn($col = 0) {
		$data = array ();
		$col = ( int ) $col;
		$row = $this->fetch ( \Core\Db\Init::FETCH_NUM );
		if (! is_array ( $row )) {
			return false;
		}
		return $row [$col];
	...
	public function fetchObject($class = 'stdClass', array $config = array()) {
		...
		$row = $this->fetch ( \Core\Db\Init::FETCH_ASSOC );
		if (! is_array ( $row )) {
			return false;
		}
	 ...
	 *        	The fetch mode.
	 * @return bool
	 * @throws \Core\Db\Init_Statement_Exception
	 */
	public function setFetchMode($mode) {
		switch ($mode) {
			...
			default :
				 ...
				 */
				require_once 'Db/Statement/Exception.php';
				throw new \Core\Db\Statement\Exception ( 'invalid fetch mode' );
				break;
		}
	}
	 ...
	 * particular \Core\Db\Statement object.
	 *
	 * @return \Core\Db\Adapter\Abs
	 */
