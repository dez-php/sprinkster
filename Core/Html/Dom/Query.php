...
class Query {
    ...
    function __construct($str=null, $lowercase=true, $forceTagsClosed=true, $target_charset=\Core\Html\Dom\Query::DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=\Core\Html\Dom\Query::DEFAULT_BR_TEXT) {
        if ($str) {
    ...
    function load($str, $lowercase=true, $stripRN=true, $defaultBRText=\Core\Html\Dom\Query::DEFAULT_BR_TEXT) {
        global $debugObject;
    ...
    function load_file() {
        ...
        // Per the \Core\Html\Dom\Query repositiry this is a planned upgrade to the codebase.
    ...
    protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=\Core\Html\Dom\Query::DEFAULT_BR_TEXT) {
        $this->clear();
        ...
        $this->root = new \Core\Html\Dom\Query\Node($this);
        $this->root->tag = 'root';
    ...
    protected function parse() {
        ...
        // text
        $node = new \Core\Html\Dom\Query\Node($this);
        ++$this->cursor;
    ...
    protected function read_tag() {
        ...
        if ($this->char==='/') {
            $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
            // This represetns the change in the \Core\Html\Dom\Query trunk from revision 180 to 181.
