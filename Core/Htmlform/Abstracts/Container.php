 ...
 */
abstract class Container extends \Core\Htmlform\Abstracts\Element {
	// {{{ variables
	/**
	 ...
	 *        	container parameters, HTML attributes
	 * @param $form (object)
	 *        	parent form object
	 * @return void
	 *
	...
	public function __call($function, $arguments) {
		if (substr ( $function, 0, 3 ) === 'add') {
			$type = strtolower ( str_replace ( 'add', '\\Core\\Htmlform\\Elements\\', $function ) );
	 ...
	 *
	 * (see @link depage::htmlform::elements::creditcard @endlink for an example
	 * implementation)
	 *
	 * @return void
	 *
	...
	public function addHtml($html) {
		$htmlElement = new \Core\Htmlform\Elements\Html ( $html );
	 ...
	 *        	array of opional parameters
	 *        	
	 */
	public function addStepNav($parameter = array()) {
		$htmlElement = new \Core\Htmlform\Elements\Stepnav ( $parameter, $this->form );
	 ...
	 *
	 * Checks if an element type class exists. Throws an exception if it
	 * doesn't.
	 *
	 * @param $type (string)
	 *        	element type
	...
	private function checkElementType($type) {
		if (! class_exists ( $type )) {
			throw new \Core\Htmlform\Exceptions\UnknownElementTypeException ();
		}
	}
	// }}}
	 ...
	 * of subelements.
	 *
	 * @param $includeFieldsets (bool)
	 *        	include fieldset/step elements in result
	...
	public function getElements($includeFieldsets = false) {
		$allElements = array ();
		foreach ( $this->elements as $element ) {
			if ($element instanceof \Core\Htmlform\Elements\Fieldset) {
	 ...
	 *
	 * @param $name (string)
	 *        	name of the input element we're looking for
