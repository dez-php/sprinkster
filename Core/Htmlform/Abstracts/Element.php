...
abstract class Element {
	 ...
	 *        	element parameters, HTML attributes
	 * @param $form (object)
	 ...
	 * It is defined on this abstract class since clearSession calls it for
	 ...
	 *
	 * @param $parameters (array)
	 *        	parameters for element constructor
	 * @return void
	 *
	 */
	protected function checkParameters($parameters) {
		if ((isset ( $parameters )) && (! is_array ( $parameters ))) {
			throw new \Core\Htmlform\Exceptions\ElementParametersNoArrayException ( 'Element "' . $this->getName () . '": parameters must be of type array.' );
		}
	}
	// }}}
	...
	private function checkName($name) {
		if (! is_string ( $name ) || trim ( $name ) === '' || preg_match ( '/[^a-zA-Z0-9_\[\]]/', $name )) {
			throw new \Core\Htmlform\Exceptions\InvalidElementNameException ( '"' . $name . '" is not a valid element name.' );
		}
	}
	// }}}
	...
	protected function htmlEscape($options = array()) {
		if (is_string ( $options )) {
			$htmlOptions = htmlspecialchars ( $options, ENT_QUOTES, 'utf-8' );
