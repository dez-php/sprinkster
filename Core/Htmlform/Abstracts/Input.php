 ...
 **/
namespace Core\Htmlform\Abstracts;
 ...
 */
abstract class Input extends \Core\Htmlform\Abstracts\Element {
	// {{{ variables
	/**
	...
	protected $required;
	/**
	 * @brief Name of the parent HTML form.
	 * Used to identify the element once it's rendered.
	 */
	protected $formName;
	/**
	 ...
	 *        	input element parameters, HTML attributes, validator specs
	 *        	etc.
	 * @param $form (object)
	 *        	parent form object
	 * @return void
	...
	public function __construct($name, $parameters, $form) {
		$this->type = strtolower ( basename ( str_replace ( '\\', DIRECTORY_SEPARATOR, get_class ( $this ) ) ) );
		$this->formName = $form->getName ();
		
		parent::__construct ( $name, $parameters, $form );
		
		$this->validator = (isset ( $parameters ['validator'] )) ? \Core\Htmlform\Validators\Validator::factory ( $parameters ['validator'], $this->log ) : \Core\Htmlform\Validators\Validator::factory ( $this->type, $this->log );
	}
	// }}}
	...
	public function validate() {
		if (! $this->validated) {
			...
			$this->valid = (($this->value !== null) && ($this->validatorCall () || $this->isEmpty ()) && (! $this->isEmpty () || ! $this->required));
		}
		return $this->valid;
	}
	// }}}
	 ...
	 *
	 * Hook method for validator call. Validator arguments can be adjusted on
	 * override.
	 *
	...
	protected function htmlClasses() {
		$classes = 'input-' . $this->htmlEscape ( basename ( $this->type ) );
	...
	protected function htmlErrorMessage() {
		if (! $this->valid && $this->value !== null && $this->errorMessage !== "") {
			$errorMessage = "<label class=\"error\">" . $this->htmlEscape ( $this->errorMessage ) . "</label>";
		} else {
			$errorMessage = "";
		}
