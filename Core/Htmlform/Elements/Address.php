 ...
 * @endcode
 */
class Address extends \Core\Htmlform\Elements\Fieldset {
	// {{{ __construct()
	/**
	...
	public function __construct($name, $parameters, $form) {
		...
		$this->defaults ['priorityCountries'] = isset ( $parameters ['priorityCountries'] ) ? $parameters ['priorityCountries'] : array ();
		
		$this->prefix = isset ( $parameters ['prefix'] ) ? rtrim ( $parameters ['prefix'], '_' ) . '_' : '';
	}
	// }}}
	...
	protected function setDefaults() {
		...
		$this->defaults ['labelAddress1'] = ("Address 1");
		$this->defaults ['labelAddress2'] = ("Address 2");
		$this->defaults ['labelCity'] = ("City");
		$this->defaults ['labelState'] = ("State");
		$this->defaults ['labelZip'] = ("Zip");
		$this->defaults ['labelCountry'] = ("Country");
	}
	// }}}
	...
	public function validate() {
		if (parent::validate () && isset ( $this->defaultCountry ) && isset ( $this->defaultState )) {
			...
			$country_states = \Core\Htmlform\Elements\State::getStates ();
			...
			if (! $this->valid) {
				$state->errorMessage = ("State and Country do not match");
				$state->valid = false;
			}
