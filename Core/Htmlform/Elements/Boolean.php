 ...
 * @endcode
 */
class Boolean extends \Core\Htmlform\Abstracts\Input {
	// {{{ setDefaults()
	/**
	...
	public function __toString() {
		...
		return "<p {$wrapperAttributes}>" . "<label>" . "<input type=\"checkbox\" name=\"{$this->name}\"{$inputAttributes} value=\"true\"{$selected}>" . "<span class=\"label\">{$label}{$marker}</span>" . "</label>" . $errorMessage . $helpMessage . "</p>\n";
	}
	// }}}
	...
	public function validate() {
		if (! $this->validated) {
			...
			$this->valid = (($this->value !== null) && ($this->validator->validate ( $this->value ) || $this->isEmpty ()) && ($this->value || ! $this->required));
		}
