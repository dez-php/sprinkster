 ...
 * @code
 * <?php/**
 * @brief HTML-multiple-choice input type i.e.
 * checkbox and select.
 *
 * Class for multiple-choice type HTML elements. Has the same return value,
 * regardless of skin type (checkbox or select).
 ...
 * @endcode
 */
class Multiple extends \Core\Htmlform\Abstracts\Input {
	// {{{ variables
	/**
	 ...
	 * @param $value (array)
	 *        	values to be marked as selected
	...
	protected function htmlList($options = null, $value = null) {
		...
		if ($this->skin === "select") {
			...
			foreach ( $options as $index => $option ) {
				...
				$list .= "<span>" . "<label>" . "<input type=\"checkbox\" name=\"{$this->name}[]\"{$inputAttributes} value=\"{$index}\"{$selected}>" . "<span>{$option}</span>" . "</label>" . "</span>";
			}
		}
		return $list;
	}
	...
	public function __toString() {
		...
		if ($this->skin === 'select') {
			...
			return "<p {$wrapperAttributes}>" . "<label>" . "<span class=\"label\">{$label}{$marker}</span>" . "<select multiple name=\"{$this->name}[]\"{$inputAttributes}>{$list}</select>" . "</label>" . $errorMessage . $helpMessage . "</p>\n";
		} else {
			// render HTML checkbox
			
			return "<p {$wrapperAttributes}>" . "<span class=\"label\">{$label}{$marker}</span>" . "<span>{$list}</span>" . $errorMessage . $helpMessage . "</p>\n";
		}
	}
	// }}}
	...
	protected function htmlInputAttributes() {
		...
		if ($this->required && $this->skin === 'select')
