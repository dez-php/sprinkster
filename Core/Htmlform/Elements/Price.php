 ...
 * @endcode
 */
class Price extends \Core\Htmlform\Elements\Text {
	// {{{ variables
	/**
	...
	public function __toString() {
		...
		$currency = $this->htmlCurrency();

		return "<p {$wrapperAttributes}>" . "<label>" . "<span class=\"label\">{$label}{$marker}</span>" . "<input name=\"{$this->name}\" type=\"{$this->type}\"{$max}{$min}{$step}{$inputAttributes} value=\"{$value}\">" . $list . "</label>" . $currency . $errorMessage . $helpMessage . "</p>\n";
	}
	// }}}
	...
	protected function htmlCurrency() {
		return ($this->currency === null) ? '' : ' ' . $this->htmlEscape( $this->currency )  . ' ';
	}
