 ...
 * @endcode
 */
class Single extends \Core\Htmlform\Abstracts\Input {
	// {{{ variables
	/**
	...
	protected function htmlList($options = null, $value = null) {
		...
		if ($this->skin === "select") {
			...
			foreach ( $options as $index => $option ) {
				...
				$list .= "<span>" . "<label>" . "<input type=\"radio\" name=\"{$this->name}\"{$inputAttributes} value=\"{$index}\"{$selected}>" . "<span>{$option}</span>" . "</label>" . "</span>";
			}
		}
		return $list;
	}
	...
	public function __toString() {
		...
		if ($this->skin === "select") {
			...
			return "<p {$wrapperAttributes}>" . "<label>" . "<span class=\"label\">{$label}{$marker}</span>" . "<select name=\"{$this->name}\"{$inputAttributes}>{$list}</select>" . "</label>" . $errorMessage . $helpMessage . "</p>\n";
		} else {
