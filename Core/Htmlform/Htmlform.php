 ...
 *
 * You can find a list of available input-class in @link
 ...
 *       element-classes.
 *       - New element-classes are automatically integrated by the autoloader.
 *       (They
 *       can be instantiated with @link
 *       depage::htmlform::abstracts::container::__call
 ...
 *      
 *       To generate a gzipped release with the essentials for working
 *       environments:
 *      
 *       <pre>$ make min</pre>
 *      
 ...
 *           
 *            All Classes in this namespace are HTML input-elements that can be
 *            added to
 *            instances of the @link depage::htmlform::htmlform
 *            htmlform-class@endlink,
 *            but also to also to @link depage::htmlform::elements::fieldset
 *            fieldsets@endlink
 ...
 *
 * // add form fields
 * $form->addText('username', array('label' => 'User name', 'required' =>
 * true));
 ...
 */
class Htmlform extends \Core\Htmlform\Abstracts\Container {
	// {{{ variables
	/**
	...
	protected $submitURL;
	/**
	 * @brief Specifies where the user is redirected to, once the form-data is
	 * valid
	 */
	...
	protected $class;
	/**
	 * @brief Contains the name of the array in the PHP session, holding the
	 * form-data
	 */
	 ...
	 *        	form parameters, HTML attributes
	 * @param $form (object)
	 *        	parent form object reference (not used in this case)
	 * @return void
	 *
	 */
	...
	private function sessionExpiry() {
		if (isset ( $this->ttl ) && is_numeric ( $this->ttl )) {
			...
			if (isset ( $this->sessionSlot ['formTimestamp'] ) && ($timestamp - $this->sessionSlot ['formTimestamp'] > $this->ttl)) {
				$this->clearSession ();
				$this->sessionSlot = & $_SESSION [$this->sessionSlotName];
			}
	 ...
	 *
	 * @param $type (string)
	 *        	input type or fieldset
	 * @param $name (string)
	 *        	name of the element
	 * @param $parameters (array)
	 *        	element attributes: HTML attributes, validation parameters
	 *        	etc.
	...
	protected function addElement($type, $name, $parameters) {
		...
		if ($newElement instanceof \Core\Htmlform\Elements\Step) {
	...
	public function updateInputValue($name) {
		...
		if (isset ( $_POST ['formName'] ) && ($_POST ['formName'] === $this->name) && $this->inCurrentStep ( $name )) {
			if ($this->getElement ( $name ) instanceof \Core\Htmlform\Elements\File) {
	...
	private function setCurrentStep() {
		if (! is_numeric ( $this->currentStepId ) || ($this->currentStepId > count ( $this->steps ) - 1) || ($this->currentStepId < 0)) {
	...
	private function getCurrentElements() {
		...
		foreach ( $this->elements as $element ) {
			if ($element instanceof elements\fieldset) {
				if (! ($element instanceof elements\step) || (isset ( $this->steps [$this->currentStepId] ) && ($element == $this->steps [$this->currentStepId]))) {
	...
	public function onlyOpen() {
		...
		foreach ( $this->elementsAndHtml as $element ) {
			...
			if (! ($element instanceof \Core\Htmlform\Elements\Step) || (isset ( $this->steps [$this->currentStepId] ) && $this->steps [$this->currentStepId] == $element)) {
	...
	public function __toString() {
		...
		return "<form id=\"{$this->name}\" name=\"{$this->name}\" class=\"depage-form {$class}\" method=\"{$method}\" action=\"{$submitURL}\" data-jsvalidation=\"{$jsValidation}\" data-jsautosave=\"{$jsAutosave}\" enctype=\"multipart/form-data\">" . "\n" . $renderedElements . "<p id=\"{$this->name}-submit\" class=\"submit\"><input type=\"submit\" name=\"formSubmit\" value=\"{$label}\"></p>" . "\n" . $cancel . "</form>";
	}
	// }}}
	...
	public function buildUrlQuery($step_query = '') {
		...
		if (isset ( $this->url ['query'] ) && $this->url ['query'] != "") {
			...
			if (isset ( $queryParts ["step"] ))
				unset ( $queryParts ["step"] );
	...
	public function getFirstInvalidStep() {
		if (count ( $this->steps ) > 0) {
			 ...
			 * directly attached to the form that's invalid.
			 * In this case we
	...
	public function validate() {
		...
		if ($this->valid && ! is_null ( $this->validator )) {
			if (is_callable ( $this->validator )) {
				...
				throw new \Core\Exception ( "The validator paramater must be callable" );
			}
		}
	 ...
	 * It can therefore be used to test autosave fields are correct without
	 * forcing
	 * the form save.
	 *
	 ...
	 *
	 * @param $data (array)
	 *        	input element names (key) and values (value)
	 * @return void
	 *
	 */
	...
	// {{{ getValuesWithLabel()
	/**
	 * @brief Gets form-data from current PHP session but also contain elemnt
	 * labels.
	 *
	...
	public function getValuesWithLabel() {
		...
		if (isset ( $values )) {
			foreach ( $values as $element => $value ) {
				...
				if ($elem)
					$values_with_label [$element] = array (
							"value" => $value,
							"label" => $elem->getLabel () 
					);
			}
