...
class Locale {
	 ...
	 *
	 * @param string|\Core\Locale $locale
	 *        	(Optional) Locale for parsing input
	 * @throws \Core\Locale_Exception When autodetection has been failed
	 */
	...
	public static function getDefault() {
		if ((self::$compatibilityMode === true) or (func_num_args () > 0)) {
			if (! self::$_breakChain) {
				...
				trigger_error ( 'You are running \Core\Locale in compatibility mode... please migrate your scripts', E_USER_NOTICE );
	 ...
	 * @throws \Core\Locale_Exception When a autolocale was given
	 * @throws \Core\Locale_Exception When a unknown locale was given
	 * @return void
	 */
	public static function setDefault($locale, $quality = 1) {
		if (($locale === 'auto') or ($locale === 'root') or ($locale === 'default') or ($locale === 'environment') or ($locale === 'browser')) {
			...
			throw new \Core\Exception ( 'Only full qualified locales can be used as default!' );
		}
		
		if (($quality < 0.1) or ($quality > 100)) {
			require_once 'Exception.php';
			throw new \Core\Exception ( "Quality must be between 0.1 and 100" );
		}
		...
		if (isset ( self::$_localeData [( string ) $locale] ) === true) {
			...
			if (isset ( self::$_localeData [$elocale [0]] ) === true) {
				...
				require_once 'Exception.php';
				throw new \Core\Exception ( "Unknown locale '" . ( string ) $locale . "' can not be set as default!" );
			}
		}
	 ...
	 *
	 * @param \Core\Locale $object
	 *        	Locale to check for equality
	 * @return boolean
	 */
	public function equals(\Core\Locale $object) {
	...
	public static function isLocale($locale, $strict = false, $compatible = true) {
		if (($locale instanceof \Core\Locale) || (is_string ( $locale ) && array_key_exists ( $locale, self::$_localeData ))) {
			return true;
		}
	 ...
	 *
	 * @param string $locale        	
	 * @throws \Core\Locale_Exception When the given locale is no locale or the
	 *         autodetection fails
	 * @return string
	 */
	public static function findLocale($locale = null) {
		if ($locale === null) {
			...
			if (\Core\Registry::isRegistered ( '\Core\Locale' )) {
				$locale = \Core\Registry::get ( '\Core\Locale' );
			}
		}
		
		if ($locale === null) {
			$locale = new \Core\Locale ();
		}
		
		if (! \Core\Locale::isLocale ( $locale, true, false )) {
			if (! \Core\Locale::isLocale ( $locale, false, false )) {
				$locale = \Core\Locale::getLocaleToTerritory ( $locale );
				
				if (empty ( $locale )) {
					require_once 'Exception.php';
					throw new \Core\Exception ( "The locale '$locale' is no known locale" );
				}
			} else {
				$locale = new \Core\Locale ( $locale );
			}
		}
	...
	private static function _prepareLocale($locale, $strict = false) {
		...
		// This can only happen when someone extends \Core\Locale and erases the
		// default
		if ($locale === null) {
			require_once 'Exception.php';
			throw new \Core\Exception ( 'Autodetection of Locale has been failed!' );
		}
