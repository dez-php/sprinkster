...
class Base {
	...
	protected function AddAnAddress($kind, $address, $name = '') {
		if (! preg_match ( '/^(to|cc|bcc|Reply-To)$/', $kind )) {
			...
			if ($this->exceptions) {
				throw new \Core\Mailer\Exception ( 'Invalid recipient array: ' . $kind );
			}
			if ($this->SMTPDebug) {
		...
		if (! self::ValidateAddress ( $address )) {
			...
			if ($this->exceptions) {
				throw new \Core\Mailer\Exception ( $this->Lang ( 'invalid_address' ) . ': ' . $address );
			}
			if ($this->SMTPDebug) {
	...
	protected function PreSend() {
		try {
			...
			if ((count ( $this->to ) + count ( $this->cc ) + count ( $this->bcc )) < 1) {
				throw new \Core\Mailer\Exception ( $this->Lang ( 'provide_address' ), self::STOP_CRITICAL );
			}
			...
			if (empty ( $this->Body )) {
				// throw new
			// \Core\Mailer\Exception($this->Lang('empty_message'),
			// self::STOP_CRITICAL);
			}
		...
		if ($this->SingleTo === true) {
			foreach ( $this->SingleToArray as $key => $val ) {
				if (! @$mail = popen ( $sendmail, 'w' )) {
					throw new \Core\Mailer\Exception ( $this->Lang ( 'execute' ) . $this->Sendmail, self::STOP_CRITICAL );
				}
				fputs ( $mail, "To: " . $val . "\n" );
				fputs ( $mail, $header );
				fputs ( $mail, $body );
	...
	protected function MailSend($header, $body) {
		...
		if (! $rt) {
			throw new \Core\Mailer\Exception ( $this->Lang ( 'instantiate' ), self::STOP_CRITICAL );
		}
		return true;
	}
	...
	protected function SmtpSend($header, $body) {
		...
		if (! $this->SmtpConnect ()) {
			throw new \Core\Mailer\Exception ( $this->Lang ( 'smtp_connect_failed' ), self::STOP_CRITICAL );
		}
		$smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
		if (! $this->smtp->Mail ( $smtp_from )) {
			throw new \Core\Mailer\Exception ( $this->Lang ( 'from_failed' ) . $smtp_from, self::STOP_CRITICAL );
		}
		...
		if (count ( $bad_rcpt ) > 0) { // Create error message for any bad addresses
			...
			throw new \Core\Mailer\Exception ( $this->Lang ( 'recipients_failed' ) . $badaddresses );
		}
		if (! $this->smtp->Data ( $header . $body )) {
			throw new \Core\Mailer\Exception ( $this->Lang ( 'data_not_accepted' ), self::STOP_CRITICAL );
		}
	...
	public function SmtpConnect() {
		if (is_null ( $this->smtp )) {
			$this->smtp = new \Core\Mailer\Smtp ();
		}
				...
				if ($this->smtp->Connect ( ($ssl ? 'ssl://' : '') . $host, $port, $this->Timeout )) {
					...
					if ($tls) {
						if (! $this->smtp->StartTLS ()) {
							throw new \Core\Mailer\Exception ( $this->Lang ( 'tls' ) );
						}
					...
					if ($this->SMTPAuth) {
						if (! $this->smtp->Authenticate ( $this->Username, $this->Password )) {
							throw new \Core\Mailer\Exception ( $this->Lang ( 'authenticate' ) );
						}
					}
				}
				$index ++;
				if (! $connection) {
					throw new \Core\Mailer\Exception ( $this->Lang ( 'connect_host' ) );
				}
			}
		} catch ( \Core\Mailer\Exception $e ) {
			$this->smtp->Reset ();
		...
		if ($this->IsError ()) {
			...
			try {
				...
				if (@openssl_pkcs7_sign ( $file, $signed, "file://" . $this->sign_cert_file, array (
					...
					@unlink ( $file );
					@unlink ( $signed );
					throw new \Core\Mailer\Exception ( $this->Lang ( "signing" ) . openssl_error_string () );
				}
			} catch ( \Core\Mailer\Exception $e ) {
				$body = '';
				if ($this->exceptions) {
	...
	public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
		try {
			if (! @is_file ( $path )) {
				throw new \Core\Mailer\Exception ( $this->Lang ( 'file_access' ) . $path, self::STOP_CONTINUE );
			}
	...
	protected function EncodeFile($path, $encoding = 'base64') {
		try {
			// if (!is_readable($path)) {
			// throw new \Core\Mailer\Exception($this->Lang('file_open') .
		...
		if ($this->attachmendImagesBody && preg_match_all ( "/(((src|background)=[\"'](.*)[\"'])|((background-image:| )\s?url\([\"'](.*)[\"']\)))/Ui", $message, $match )) {
			...
			if ($images) {
				foreach ( $images as $i => $image ) {
					...
					$mimeType = \Core\File\Ext::getMimeFromExt ( $ext );
	...
	public function set($name, $value = '') {
		try {
			if (isset ( $this->$name )) {
				...
				throw new \Core\Mailer\Exception ( $this->Lang ( 'variable_set' ) . $name, self::STOP_CRITICAL );
			}
		} catch ( \Exception $e ) {
			$this->SetError ( $e->getMessage () );
			if ($e->getCode () == self::STOP_CRITICAL) {
