...
class Detect {
	...
	protected $tabletDevices = array (
			...
			'iPad' => 'iPad|iPad.*Mobile', // @todo: check for mobile
			                                            // friendly emails topic.
	...
	public function setMobileDetectionRulesExtended() {
		...
		// Merge all rules together.
		$this->mobileDetectionRules = array_merge ( $this->phoneDevices, $this->tabletDevices, $this->operatingSystems, $this->userAgents, $this->utilities );
	}
	public function checkHttpHeadersForMobile() {
		if (isset ( $this->httpHeaders ['HTTP_X_WAP_PROFILE'] ) || isset ( $this->httpHeaders ['HTTP_X_WAP_CLIENTID'] ) || isset ( $this->httpHeaders ['HTTP_WAP_CONNECTION'] ) || isset ( $this->httpHeaders ['HTTP_PROFILE'] ) || isset ( $this->httpHeaders ['HTTP_X_OPERAMINI_PHONE_UA'] ) || 		// Reported
		                                                               // by Nokia
		                                                               // devices (eg.
		                                                               // C3)
		isset ( $this->httpHeaders ['HTTP_X_NOKIA_IPADDRESS'] ) || isset ( $this->httpHeaders ['HTTP_X_NOKIA_GATEWAY_ID'] ) || isset ( $this->httpHeaders ['HTTP_X_ORANGE_ID'] ) || isset ( $this->httpHeaders ['HTTP_X_VODAFONE_3GPDPCONTEXT'] ) || isset ( $this->httpHeaders ['HTTP_X_HUAWEI_USERID'] ) || isset ( $this->httpHeaders ['HTTP_UA_OS'] ) || 		// Reported
		                                                               // by Windows
		                                                               // Smartphones.
		isset ( $this->httpHeaders ['HTTP_X_MOBILE_GATEWAY'] ) || 		// Reported
		                                                               // by Verizon,
		                                                               // Vodafone
		                                                               // proxy
		                                                               // system.
		(isset ( $this->httpHeaders ['HTTP_UA_CPU'] ) && $this->httpHeaders ['HTTP_UA_CPU'] == 'ARM') || 		// Seen
		                                                               // this on a
		                                                               // HTC.
		isset ( $this->httpHeaders ['HTTP_ACCEPT'] ) && (strpos ( $this->httpHeaders ['HTTP_ACCEPT'], 'text/vnd.wap.wml' ) !== false || strpos ( $this->httpHeaders ['HTTP_ACCEPT'], 'application/vnd.wap.xhtml+xml' ) !== false)) {
	...
	private function matchUAAgainstKey($key, $userAgent = null) {
		...
		// Make the keys lowercase so we can match: isIphone(), isiPhone(),
		// isiphone(), etc.
		$key = strtolower ( $key );
