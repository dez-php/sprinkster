...
class Docblock implements \Reflector {
	...
	public function __construct($commentOrReflector) {
		if ($commentOrReflector instanceof Reflector) {
			...
			if (! method_exists ( $commentOrReflector, 'getDocComment' )) {
				...
				throw new \Core\Reflection\Exception ( 'Reflector must contain method "getDocComment"' );
			}
			...
			require_once 'Reflection/Exception.php';
			throw new \Core\Reflection\Exception ( get_class ( $this ) . ' must have a (string) DocComment or a Reflector in the constructor' );
		}
		
		if ($docComment == '') {
			require_once 'Reflection/Exception.php';
			throw new \Core\Reflection\Exception ( 'DocComment cannot be empty' );
		}
	 ...
	 *
	 * @param string $name        	
	 * @return \Core\Reflection\Docblock_Tag false
	 */
	 ...
	 *
	 * @param string $filter        	
	 * @return array Array of \Core\Reflection\Docblock_Tag
	 */
	...
	protected function _parse() {
		...
		while ( ($newlinePos = strpos ( $parsedDocComment, "\n" )) !== false ) {
			...
			if ((strpos ( $line, '@' ) === 0) && (preg_match ( '#^(@\w+.*?)(\n)(?:@|\r?\n|$)#s', $parsedDocComment, $matches ))) {
				$this->_tags [] = \Core\Reflection\Docblock_Tag::factory ( $matches [1] );
