...
class File implements \Reflector {
	...
	/**
	 *
	 * @var \Core\Reflection\Classes[]
	 */
	protected $_classes = array ();
	
	/**
	 *
	 * @var \Core\Reflection\Functions[]
	 */
	...
	public function __construct($file) {
		...
		if (! $fileRealpath || ! in_array ( $fileRealpath, get_included_files () )) {
			...
			throw new \Core\Reflection\Exception ( 'File ' . $file . ' must be required before it can be reflected' );
		}
	...
	public static function findRealpathInIncludePath($fileName) {
		...
		$includePaths = \Core\Loader\Loader::explodeIncludePath ();
	 ...
	 * @return array Array of \Core\Reflection\Classes instances
	 */
	 ...
	 * @param string $reflectionClass
	 *        	Name of reflection class to use for instances
	 * @return array Array of \Core\Reflection\Functionss
	 */
	 ...
	 * @throws \Core\Reflection\Exception for invalid class name or invalid
	 *         reflection class
	 */
	public function getClass($name = null, $reflectionClass = '\Core\Reflection\Classes') {
		if ($name === null) {
			reset ( $this->_classes );
			...
			if (! $instance instanceof \Core\Reflection\Classes) {
				require_once 'Reflection/Exception.php';
				throw new \Core\Reflection\Exception ( 'Invalid reflection class given; must extend \Core\Reflection\Classes' );
			}
			return $instance;
		}
		...
		require_once 'Reflection/Exception.php';
		throw new \Core\Reflection\Exception ( 'Class by name ' . $name . ' not found.' );
	}
	...
	protected function _reflect() {
		...
		foreach ( $tokens as $token ) {
			/*
			 * Tokens are characters representing symbols or arrays representing
			 * strings. The keys/values in the arrays are - 0 => token id, - 1
			 * => string, - 2 => line number Token ID's are explained here:
