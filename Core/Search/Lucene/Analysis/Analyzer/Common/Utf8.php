...
class Utf8 extends \Core\Search\Lucene\Analysis\Analyzer\Common {
	/**
	...
	public function __construct() {
		if (@preg_match ( '/\pL/u', 'a' ) != 1) {
			...
			throw new \Core\Search\Lucene\Exception ( 'Utf8 analyzer needs PCRE unicode support to be enabled.' );
		}
	}
	...
	public function reset() {
		...
		if (strcasecmp ( $this->_encoding, 'utf8' ) != 0 && strcasecmp ( $this->_encoding, 'utf-8' ) != 0) {
			$this->_input = mb_convert_encoding ( $this->_value, 'UTF-8', $this->_encoding );
	...
	public function nextToken() {
		...
		do {
			...
			$startPos = $this->_position + mb_strlen ( substr ( $this->_input, $this->_bytePosition, $binStartPos - $this->_bytePosition ), 'UTF-8' );
