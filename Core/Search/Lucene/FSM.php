...
abstract class FSM {
	 ...
	 *
	 * @param integer|string $state        	
	 * @throws \Core\Search\Exception
	 */
	public function setState($state) {
		if (! isset ( $this->_states [$state] )) {
			throw new \Core\Search\Exception ( 'State \'' . $state . '\' is not on of the possible FSM states.' );
		}
	 ...
	 * @param \Core\Search\Lucene\FSMAction|null $inputAction        	
	 * @throws \Core\Search\Exception
	 */
	public function addRule($sourceState, $input, $targetState, $inputAction = null) {
		if (! isset ( $this->_states [$sourceState] )) {
			throw new \Core\Search\Exception ( 'Undefined source state (' . $sourceState . ').' );
		}
		if (! isset ( $this->_states [$targetState] )) {
			throw new \Core\Search\Exception ( 'Undefined target state (' . $targetState . ').' );
		}
		if (! isset ( $this->_inputAphabet [$input] )) {
			throw new \Core\Search\Exception ( 'Undefined input symbol (' . $input . ').' );
		}
		...
		if (isset ( $this->_rules [$sourceState] [$input] )) {
			throw new \Core\Search\Exception ( 'Rule for {state,input} pair (' . $sourceState . ', ' . $input . ') is already defined.' );
		}
	 ...
	 *
	 * @param integer|string $state        	
	 * @param \Core\Search\Lucene\FSMAction $action        	
	 */
	public function addEntryAction($state,\Core\Search\Lucene\FSMAction $action) {
		if (! isset ( $this->_states [$state] )) {
			throw new \Core\Search\Exception ( 'Undefined state (' . $state . ').' );
		}
	 ...
	 * @param \Core\Search\Lucene\FSMAction $action        	
	 */
	public function addExitAction($state,\Core\Search\Lucene\FSMAction $action) {
		if (! isset ( $this->_states [$state] )) {
			throw new \Core\Search\Exception ( 'Undefined state (' . $state . ').' );
		}
	 ...
	 * @param \Core\Search\Lucene\FSMAction $action        	
	 */
	public function addInputAction($state, $inputSymbol,\Core\Search\Lucene\FSMAction $action) {
		if (! isset ( $this->_states [$state] )) {
			throw new \Core\Search\Exception ( 'Undefined state (' . $state . ').' );
		}
		if (! isset ( $this->_inputAphabet [$inputSymbol] )) {
			throw new \Core\Search\Exception ( 'Undefined input symbol (' . $inputSymbol . ').' );
		}
	 ...
	 * @param \Core\Search\Lucene\FSMAction $action        	
	 */
	public function addTransitionAction($sourceState, $targetState,\Core\Search\Lucene\FSMAction $action) {
		if (! isset ( $this->_states [$sourceState] )) {
			throw new \Core\Search\Exception ( 'Undefined source state (' . $sourceState . ').' );
		}
		if (! isset ( $this->_states [$targetState] )) {
			throw new \Core\Search\Exception ( 'Undefined source state (' . $targetState . ').' );
		}
	...
	public function process($input) {
		if (! isset ( $this->_rules [$this->_currentState] )) {
			throw new \Core\Search\Exception ( 'There is no any rule for current state (' . $this->_currentState . ').' );
		}
		if (! isset ( $this->_rules [$this->_currentState] [$input] )) {
			throw new \Core\Search\Exception ( 'There is no any rule for {current state, input} pair (' . $this->_currentState . ', ' . $input . ').' );
		}
	...
	public function reset() {
		if (count ( $this->_states ) == 0) {
			throw new \Core\Search\Exception ( 'There is no any state defined for FSM.' );
		}
