...
class DictionaryLoader
{
     ...
     *
     * See \Core\Search\Lucene\Index\SegmintInfo class for details
     *
     * @param string $data
     * @return array
    ...
    public static function load($data)
    {
        ...
        if ($tiVersion != (int)0xFFFFFFFE /* pre-2.1 format */ &&
            ...
            throw new \Core\Search\Lucene\Exception('Wrong TermInfoIndexFile file format');
        }
        ...
        if (PHP_INT_SIZE > 4) {
            ...
            if ((ord($data[$pos])            != 0) ||
                ...
                ((ord($data[$pos+4]) & 0x80) != 0)) {
                     throw new \Core\Search\Lucene\Exception('Largest supported segment size (for 32-bit mode) is 2Gb');
                 }
        ...
        $pos += 4;
        if ($indexTermCount < 1) {
            throw new \Core\Search\Lucene\Exception('Wrong number of terms in a term dictionary index');
        }
        ...
        for ($count = 0; $count < $indexTermCount; $count++) {
            ...
            // $termValue        = \Core\Search\Lucene\Index\Term::getPrefix($prevTerm, $termPrefixLength) . $termSuffix;
            $pb = 0; $pc = 0;
            ...
            // $this->_termDictionary[] =  new \Core\Search\Lucene\Index\Term($termValue, $termFieldNum);
            $termDictionary[] = array($termFieldNum, $termValue);

            $termInfos[] =
                 // new \Core\Search\Lucene\Index\TermInfo($docFreq, $freqPointer, $proxPointer, $skipDelta, $indexPointer);
