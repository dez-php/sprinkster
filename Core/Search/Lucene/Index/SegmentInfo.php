...
class SegmentInfo {
	 ...
	 *
	 * Array of arrays (\Core\Search\Lucene\Index\Term objects are represented
	 * as arrays because
	 ...
	 *
	 * Array of arrays (\Core\Search\Lucene\Index\TermInfo objects are
	...
	/**
	 * Segment fields.
	 * Array of \Core\Search\Lucene\Index\FieldInfo objects for this segment
	 *
	 * @var array
	 */
	private $_fields;
	 ...
	 * File system adapter.
	 *
	 * @var \Core\Search\Lucene\Storage\Directory\Filesystem
	 */
	private $_directory;
	 ...
	 * \Core\Search\Lucene\Search\Similarity::encodeNorm())
	 *
	 * @var array
	 */
	 ...
	 * items: - 'offset' => $docStoreOffset The starting document in the shared
	 * doc store files where this segment's documents begin - 'segment' =>
	 * $docStoreSegment The name of the segment that has the shared doc store
	 * files. - 'isCompound' => $docStoreIsCompoundFile True, if compound file
	 * format is used for the shared doc store files (.cfx file).
	 */
	private $_sharedDocStoreOptions;
	
	/**
	 * \Core\Search\Lucene\Index\SegmentInfo constructor
	 *
	 ...
	 */
	public function __construct(\Core\Search\Lucene\Storage\Directory $directory, $name, $docCount, $delGen = 0, $docStoreOptions = null, $hasSingleNormFile = false, $isCompound = null) {
		...
		for($count = 0; $count < $fieldsCount; $count ++) {
			...
			$this->_fields [$count] = new \Core\Search\Lucene\Index\FieldInfo ( $fieldName, $fieldBits & 1, $count, $fieldBits & 2 );
			if ($fieldBits & 0x10) {
				// norms are omitted for the indexed field
				$this->_norms [$count] = str_repeat ( chr ( \Core\Search\Lucene\Search\Similarity::encodeNorm ( 1.0 ) ), $docCount );
			}
	...
	public function openCompoundFile($extension, $shareHandler = true) {
		if (($extension == '.fdx' || $extension == '.fdt') && $this->_usesSharedDocStore) {
			...
			if (! isset ( $this->_sharedDocStoreOptions ['files'] [$fdxFName] )) {
				throw new \Core\Search\Lucene\Exception ( 'Shared doc storage segment compound file doesn\'t contain ' . $fdxFName . ' file.' );
			}
			if (! isset ( $this->_sharedDocStoreOptions ['files'] [$fdtFName] )) {
				throw new \Core\Search\Lucene\Exception ( 'Shared doc storage segment compound file doesn\'t contain ' . $fdtFName . ' file.' );
			}
		...
		if (! isset ( $this->_segFiles [$filename] )) {
			throw new \Core\Search\Lucene\Exception ( 'Segment compound file doesn\'t contain ' . $filename . ' file.' );
		}
	...
	public function compoundFileLength($extension) {
		if (($extension == '.fdx' || $extension == '.fdt') && $this->_usesSharedDocStore) {
			...
			if (! isset ( $this->_sharedDocStoreOptions ['fileSizes'] [$filename] )) {
				throw new \Core\Search\Lucene\Exception ( 'Shared doc store compound file doesn\'t contain ' . $filename . ' file.' );
			}
		...
		if (! isset ( $this->_segFileSizes [$filename] )) {
			throw new \Core\Search\Lucene\Exception ( 'Index compound file doesn\'t contain ' . $filename . ' file.' );
		}
	 ...
	 * @return \Core\Search\Lucene\Index\FieldInfo
	 */
	...
	private function _loadDictionaryIndex() {
		...
		// Load dictionary index data
		list ( $this->_termDictionary, $this->_termDictionaryInfos ) = \Core\Search\Lucene\Index\DictionaryLoader::load ( $tiiFileData );
	 ...
	 * @return \Core\Search\Lucene\Index\TermInfo
	 */
	public function getTermInfo(\Core\Search\Lucene\Index\Term $term) {
		$termKey = $term->key ();
		...
		while ( $highIndex >= $lowIndex ) {
			...
			if ($delta < 0) {
				...
				$termInfo = new \Core\Search\Lucene\Index\TermInfo ( $a [0], $a [1], $a [2], $a [3], $a [4] );
		...
		if ($tiVersion != ( int ) 0xFFFFFFFE /* pre-2.1 format */  &&
			...
			throw new \Core\Search\Lucene\Exception ( 'Wrong TermInfoFile file format' );
		}
		...
		for($count = $prevPosition * $indexInterval + 1; $count <= $termCount && ($this->_getFieldPosition ( $termFieldNum ) < $searchDicField || ($this->_getFieldPosition ( $termFieldNum ) == $searchDicField && strcmp ( $termValue, $term->text ) < 0)); $count ++) {
			...
			$termValue = \Core\Search\Lucene\Index\Term::getPrefix ( $termValue, $termPrefixLength ) . $termSuffix;
		...
		if ($termFieldNum == $searchField && $termValue == $term->text) {
			$termInfo = new \Core\Search\Lucene\Index\TermInfo ( $docFreq, $freqPointer, $proxPointer, $skipOffset );
		} else {
			$termInfo = null;
		}
	 ...
	 * @return \Core\Search\Lucene\Index\TermInfo
	 */
	public function termFreqs(\Core\Search\Lucene\Index\Term $term, $shift = 0) {
	 ...
	 * @return \Core\Search\Lucene\Index\TermInfo
	 */
	public function termPositions(\Core\Search\Lucene\Index\Term $term, $shift = 0) {
		$termInfo = $this->getTermInfo ( $term );
	...
	private function _loadNorm($fieldNum) {
		if ($this->_hasSingleNormFile) {
			...
			if ($header != 'NRM' || $headerFormatVersion != ( int ) 0xFF) {
				throw new \Core\Search\Lucene\Exception ( 'Wrong norms file format.' );
			}
	...
	public function norm($id, $fieldName) {
		...
		return \Core\Search\Lucene\Search\Similarity::decodeNorm ( ord ( $this->_norms [$fieldNum] [$id] ) );
	}
	 ...
	 *
	 * @var \Core\Search\Lucene\Storage\File
	 */
	 ...
	 *
	 * @var \Core\Search\Lucene\Index\TermInfo
	 */
	...
	const SM_TERMS_ONLY = 0; // terms are scanned, no additional info is
	                         // retrieved
	const SM_FULL_INFO = 1; // terms are scanned, frequency and position info is
	                         // retrieved
	const SM_MERGE_INFO = 2; // terms are scanned, frequency and position info is
	                         // retrieved
	...
	public function reset($startId = 0, $mode = self::SM_TERMS_ONLY) {
		...
		$this->_lastTerm = new \Core\Search\Lucene\Index\Term ( '', - 1 );
		$this->_lastTermInfo = new \Core\Search\Lucene\Index\TermInfo ( 0, 0, 0, 0 );
		...
		switch ($mode) {
			...
			default :
				throw new \Core\Search\Lucene\Exception ( 'Wrong terms scaning mode specified.' );
				break;
		}
	...
	public function skipTo(\Core\Search\Lucene\Index\Term $prefix) {
		...
		$this->_lastTerm = new \Core\Search\Lucene\Index\Term ( $prevTerm [1] /* text */,
                                                                 ($prevTerm [0] == - 1) ? '' : $this->_fields [$prevTerm [0] /* field */]->name );
		$this->_lastTermInfo = new \Core\Search\Lucene\Index\TermInfo ( $prevTermInfo [0] /* docFreq */,
	...
	public function nextTerm() {
		...
		$termValue = \Core\Search\Lucene\Index\Term::getPrefix ( $this->_lastTerm->text, $termPrefixLength ) . $termSuffix;
		
		$this->_lastTerm = new \Core\Search\Lucene\Index\Term ( $termValue, $this->_fields [$termFieldNum]->name );
		...
		$this->_lastTermInfo = new \Core\Search\Lucene\Index\TermInfo ( $docFreq, $freqPointer, $proxPointer, $skipOffset );
