...
abstract class SegmentWriter {
	...
	public function addField(\Core\Search\Lucene\Field $field) {
		if (! isset ( $this->_fields [$field->name] )) {
			...
			$this->_fields [$field->name] = new \Core\Search\Lucene\Index\FieldInfo ( $field->name, $field->isIndexed, $fieldNumber, $field->storeTermVector );
	 ...
	 * Returns actual field number
	 *
	 * @param \Core\Search\Lucene\Index\FieldInfo $fieldInfo        	
	 * @return integer
	 */
	public function addFieldInfo(\Core\Search\Lucene\Index\FieldInfo $fieldInfo) {
		if (! isset ( $this->_fields [$fieldInfo->name] )) {
			$fieldNumber = count ( $this->_fields );
			$this->_fields [$fieldInfo->name] = new \Core\Search\Lucene\Index\FieldInfo ( $fieldInfo->name, $fieldInfo->isIndexed, $fieldNumber, $fieldInfo->storeTermVector );
	 ...
	 *        	array of \Core\Search\Lucene\Field objects
	 */
...
// not supported yet            0x08 /* term offsets are stored with the term vectors */   |
	 ...
	 *
	 * @param \Core\Search\Lucene\Index\Term $termEntry        	
	...
	public function addTerm($termEntry, $termDocs) {
		...
		$term = new \Core\Search\Lucene\Index\Term ( $termEntry->text, $this->_fields [$termEntry->field]->number );
		$termInfo = new \Core\Search\Lucene\Index\TermInfo ( count ( $termDocs ), $freqPointer, $proxPointer, $skipOffset );
	 ...
	 *
	 * @param \Core\Search\Lucene\Storage\File $dicFile        	
	 * @param \Core\Search\Lucene\Index\Term $prevTerm        	
	 * @param \Core\Search\Lucene\Index\Term $term        	
	 * @param \Core\Search\Lucene\Index\TermInfo $prevTermInfo        	
	 * @param \Core\Search\Lucene\Index\TermInfo $termInfo        	
	 */
	protected function _dumpTermDictEntry(\Core\Search\Lucene\Storage\File $dicFile, &$prevTerm, \Core\Search\Lucene\Index\Term $term, &$prevTermInfo,\Core\Search\Lucene\Index\TermInfo $termInfo) {
