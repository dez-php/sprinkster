...
class Writer {
	 ...
	 * @todo Implement \Core\Search\Lucene\Storage\DirectoryRAM and
	 *       \Core\Search\Lucene\Storage\FileRAM to use it for
	 ...
	 * Array of \Core\Search\Lucene\Index\SegmentInfo objects
	 *
	 * @var array
	 */
	 ...
	 *
	 * @var \Core\Search\Lucene\Index\SegmentWriter\DocumentWriter
	 */
	 ...
	 *
	 * It's a reference to the corresponding \Core\Search\Lucene::$_segmentInfos
	 * array
	 *
	 ...
	 */
	public static function createIndex(\Core\Search\Lucene\Storage\Directory $directory, $generation, $nameCount) {
		if ($generation == 0) {
			...
			$segmentsFile = $directory->createFile ( \Core\Search\Lucene::getSegmentFileName ( $generation ) );
	 ...
	 * @param \Core\Search\Lucene\Storage\File $cleanUpLock        	
	 */
	public function __construct(\Core\Search\Lucene\Storage\Directory $directory, &$segmentInfos, $targetFormatVersion) {
	...
	public function addDocument(\Core\Search\Lucene\Document $document) {
		if ($this->_currentSegment === null) {
			$this->_currentSegment = new \Core\Search\Lucene\Index\SegmentWriter\DocumentWriter ( $this->_directory, $this->_newSegmentName () );
		}
	...
	private function _maybeMergeSegments() {
		if (\Core\Search\Lucene\LockManager::obtainOptimizationLock ( $this->_directory ) === false) {
			return;
		}
		
		if (! $this->_hasAnythingToMerge ()) {
			\Core\Search\Lucene\LockManager::releaseOptimizationLock ( $this->_directory );
			return;
		}
	...
	private function _mergeSegments($segments) {
		...
		$merger = new \Core\Search\Lucene\Index\SegmentMerger ( $this->_directory, $newName );
	...
	private function _updateSegments() {
		...
		$generation = \Core\Search\Lucene::getActualGeneration ( $this->_directory );
		$segmentsFile = $this->_directory->getFileObject ( \Core\Search\Lucene::getSegmentFileName ( $generation ), false );
		$newSegmentFile = $this->_directory->createFile ( \Core\Search\Lucene::getSegmentFileName ( ++ $generation ), false );
		...
		try {
			// Write format marker
			if ($this->_targetFormatVersion == \Core\Search\Lucene::FORMAT_2_1) {
				$newSegmentFile->writeInt ( ( int ) 0xFFFFFFFD );
			} else if ($this->_targetFormatVersion == \Core\Search\Lucene::FORMAT_2_3) {
			...
			if ($format == ( int ) 0xFFFFFFFF) {
				$srcFormat = \Core\Search\Lucene::FORMAT_PRE_2_1;
			} else if ($format == ( int ) 0xFFFFFFFD) {
				$srcFormat = \Core\Search\Lucene::FORMAT_2_1;
			} else if ($format == ( int ) 0xFFFFFFFC) {
				$srcFormat = \Core\Search\Lucene::FORMAT_2_3;
			} else {
			...
			for($count = 0; $count < $segmentsCount; $count ++) {
				...
				if ($srcFormat == \Core\Search\Lucene::FORMAT_PRE_2_1) {
					...
					if ($srcFormat == \Core\Search\Lucene::FORMAT_2_3) {
				...
				if (! in_array ( $segName, $this->_segmentsToDelete )) {
					...
					if ($this->_targetFormatVersion == \Core\Search\Lucene::FORMAT_2_3) {
						...
						throw new \Core\Search\Lucene\Exception ( 'Index conversion to lower format version is not supported.' );
					}
		...
		if (\Core\Search\Lucene\LockManager::escalateReadLock ( $this->_directory )) {
			/**
			 * Clean-up directory
			 */
			$filesToDelete = array ();
			...
			foreach ( $this->_directory->fileList () as $file ) {
				if ($file == 'deletable') {
					// 'deletable' file
					$filesToDelete [] = $file;
					$filesTypes [] = 0; // delete this file first, since it's not
					                      // used starting from Lucene v2.1
					$filesNumbers [] = 0;
				} else if ($file == 'segments') {
					// 'segments' file
					$filesToDelete [] = $file;
					$filesTypes [] = 1; // second file to be deleted "zero" version
					                      // of segments file (Lucene pre-2.1)
					$filesNumbers [] = 0;
				} else if (preg_match ( '/^segments_[a-zA-Z0-9]+$/i', $file )) {
					// 'segments_xxx' file
					// Check if it's not a just created generation file
					if ($file != \Core\Search\Lucene::getSegmentFileName ( $generation )) {
						$filesToDelete [] = $file;
						$filesTypes [] = 2; // first group of files for deletions
						$filesNumbers [] = ( int ) base_convert ( substr ( $file, 9 ), 36, 10 ); // ordered
						                                                                // by
						                                                                // segment
						                                                                // generation
						                                                                // numbers
					...
					// Check if it's not one of the segments in the current
					// segments set
					if (! isset ( $segments [$matches [1]] )) {
						$filesToDelete [] = $file;
						$filesTypes [] = 3; // second group of files for deletions
						$filesNumbers [] = ( int ) base_convert ( $matches [2], 36, 10 ); // order
						                                                           // by segment
						                                                           // number
					}
				} else if (preg_match ( '/(^_([a-zA-Z0-9]+))(_([a-zA-Z0-9]+))\.del$/i', $file, $matches )) {
					// one of per segment files
					// ('<segment_name>_<del_generation>.del' where
					// <segment_name> is '_<segment_number>')
					// Check if it's not one of the segments in the current
					// segments set
					if (! isset ( $segments [$matches [1]] )) {
						$filesToDelete [] = $file;
					...
					if (! isset ( $segments [$segmentName] ) && ($this->_currentSegment === null || $this->_currentSegment->getName () != $segmentName)) {
						$filesToDelete [] = $file;
						$filesTypes [] = 3; // second group of files for deletions
						$filesNumbers [] = ( int ) base_convert ( substr ( $file, 1 /* skip '_' */, strlen ( $file ) - 5 ), 36, 10 ); // order
						                                                                                                // by
						                                                                                                // segment
						                                                                                                // number
					}
			...
			foreach ( $delFiles as $segmentNumber => $segmentDelFiles ) {
				...
				array_pop ( $delFiles [$segmentNumber] ); // remove last delete file
				                                      // generation from candidates for
				                                      // deleting
			...
			foreach ( $delFiles as $segmentNumber => $segmentDelFiles ) {
				foreach ( $segmentDelFiles as $delGeneration => $file ) {
					...
					$filesNumbers [] = $segmentNumber * $maxGenNumber + $delGeneration; // order
					                                                                  // by
			...
			\Core\Search\Lucene\LockManager::deEscalateReadLock ( $this->_directory );
		} else {
