...
class Boolean extends \Core\Search\Lucene\Search\Query {
	
	/**
	 * Subqueries
	 * Array of \Core\Search\Lucene\Search\Query
	 *
	 * @var array
	 */
	 ...
	 *        	Array of JO_Search_Search_Query objects
	 * @param array $signs
	...
	/**
	 * Add a $subquery (\Core\Search\Lucene\Search\Query) to this query.
	 *
	 ...
	 *
	 * @param \Core\Search\Lucene\Search\Query $subquery        	
	 * @param boolean|null $sign        	
	 * @return void
	 */
	public function addSubquery(\Core\Search\Lucene\Search\Query $subquery, $sign = null) {
		if ($sign !== true || $this->_signs !== null) { // Skip, if all subqueries
		                                                // are required
			if ($this->_signs === null) { // Check, If all previous subqueries are
			                              // required
				$this->_signs = array ();
	...
	public function optimize(\Core\Search\Lucene\InterfaceLucene $index) {
		...
		foreach ( $subqueries as $id => $subquery ) {
			if ($subquery instanceof \Core\Search\Lucene\Search\Query\Insignificant) {
		...
		foreach ( $subqueries as $id => $subquery ) {
			if ($subquery instanceof \Core\Search\Lucene\Search\Query\EmptyQuery) {
				if ($signs [$id] === true) {
		...
		$optimizedQuery = new \Core\Search\Lucene\Search\Query\Boolean ( $subqueries, $signs );
		...
		foreach ( $subqueries as $id => $subquery ) {
			if ($subquery instanceof \Core\Search\Lucene\Search\Query\Term) {
				...
				unset ( $subqueries [$id] );
				unset ( $signs [$id] );
			} else if ($subquery instanceof \Core\Search\Lucene\Search\Query\MultiTerm) {
				...
				if ($signs [$id] === true) {
					...
					// Multi-term required subquery can be decomposed only if it
					// contains
					...
					// Continue if subquery has prohibited terms or doesn't have
					// required terms
		...
		if (count ( $subqueries ) == 0 && count ( array_unique ( $boostFactors ) ) == 1) {
			$optimizedQuery = new \Core\Search\Lucene\Search\Query\MultiTerm ( $terms, $tsigns );
		...
		if (count ( $terms ) == 1) {
			$clause = new \Core\Search\Lucene\Search\Query\Term ( reset ( $terms ) );
			...
			$clause = new \Core\Search\Lucene\Search\Query\MultiTerm ( $terms, $tsigns );
			$clause->setBoost ( reset ( $boostFactors ) );
		...
		if (count ( $prohibitedTerms ) == 1) {
			...
			$subqueries [] = new \Core\Search\Lucene\Search\Query\Term ( reset ( $prohibitedTerms ) );
			$signs [] = false;
			...
			foreach ( $prohibitedTerms as $id => $term ) {
				// all prohibited term are grouped as optional into multi-term
				// query
				$prohibitedSigns [$id] = null;
			}
			
			// (boost factors are not significant for prohibited clauses)
			$subqueries [] = new \Core\Search\Lucene\Search\Query\MultiTerm ( $prohibitedTerms, $prohibitedSigns );
	...
	public function createWeight(\Core\Search\Lucene\InterfaceLucene $reader) {
		$this->_weight = new \Core\Search\Lucene\Search\Weight\Boolean ( $this, $reader );
		return $this->_weight;
	}
	 ...
	 * @return float
	 */
	public function _conjunctionScore($docId,\Core\Search\Lucene\InterfaceLucene $reader) {
		if ($this->_coord === null) {
	 ...
	 * @return float
	 */
	public function _nonConjunctionScore($docId,\Core\Search\Lucene\InterfaceLucene $reader) {
		if ($this->_coord === null) {
			$this->_coord = array ();
