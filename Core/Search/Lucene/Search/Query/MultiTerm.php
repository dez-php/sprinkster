...
class MultiTerm extends \Core\Search\Lucene\Search\Query {
	
	/**
	 * Terms to find.
	 * Array of \Core\Search\Lucene\Index\Term
	 *
	 * @var array
	 */
	...
	/**
	 * Add a $term (\Core\Search\Lucene\Index\Term) to this query.
	 *
	 ...
	 * @return void
	 */
	public function addTerm(\Core\Search\Lucene\Index\Term $term, $sign = null) {
		if ($sign !== true || $this->_signs !== null) { // Skip, if all terms are
		                                                // required
	...
	public function rewrite(\Core\Search\Lucene\InterfaceLucene $index) {
		...
		if ($allQualified) {
			...
			foreach ( $this->_terms as $termId => $term ) {
				$subquery = new \Core\Search\Lucene\Search\Query\Term ( $term );
	...
	public function optimize(\Core\Search\Lucene\InterfaceLucene $index) {
		...
		if (count ( $terms ) == 1) {
			...
			$optimizedQuery = new \Core\Search\Lucene\Search\Query\Term ( reset ( $terms ) );
		...
		$optimizedQuery = new \Core\Search\Lucene\Search\Query\MultiTerm ( $terms, $signs );
		$optimizedQuery->setBoost ( $this->getBoost () );
		return $optimizedQuery;
	}
	...
	public function createWeight(\Core\Search\Lucene\InterfaceLucene $reader) {
		$this->_weight = new \Core\Search\Lucene\Search\Weight\MultiTerm ( $this, $reader );
		return $this->_weight;
	}
	 ...
	 * @param \Core\Search\Lucene\InterfaceLucene $reader        	
	 */
	private function _calculateConjunctionResult(\Core\Search\Lucene\InterfaceLucene $reader) {
	 ...
	 * @param \Core\Search\Lucene\InterfaceLucene $reader        	
	 */
	private function _calculateNonConjunctionResult(\Core\Search\Lucene\InterfaceLucene $reader) {
