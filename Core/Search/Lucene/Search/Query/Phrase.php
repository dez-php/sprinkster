...
class Phrase extends \Core\Search\Lucene\Search\Query {
	/**
	 * Terms to find.
	...
	public function __construct($terms = null, $offsets = null, $field = null) {
		...
		if (is_array ( $terms )) {
			...
			foreach ( $terms as $termId => $termText ) {
				$this->_terms [$termId] = ($field !== null) ? new \Core\Search\Lucene\Index\Term ( $termText, $field ) : new \Core\Search\Lucene\Index\Term ( $termText );
			}
		} else if ($terms === null) {
			$this->_terms = array ();
		} else {
			throw new \Core\Search\Lucene\Exception ( 'terms argument must be array of strings or null' );
		}
		
		if (is_array ( $offsets )) {
			if (count ( $this->_terms ) != count ( $offsets )) {
				throw new \Core\Search\Lucene\Exception ( 'terms and offsets arguments must have the same size.' );
			}
			...
			throw new \Core\Search\Lucene\Exception ( 'offsets argument must be array of strings or null' );
		}
	}
	 ...
	 */
	public function addTerm(\Core\Search\Lucene\Index\Term $term, $position = null) {
		if ((count ( $this->_terms ) != 0) && (end ( $this->_terms )->field != $term->field)) {
			throw new \Core\Search\Lucene\Exception ( 'All phrase terms must be in the same field: ' . $term->field . ':' . $term->text );
		}
	...
	public function rewrite(\Core\Search\Lucene\InterfaceLucene $index) {
		if (count ( $this->_terms ) == 0) {
			...
			foreach ( $index->getFieldNames ( true ) as $fieldName ) {
				$subquery = new \Core\Search\Lucene\Search\Query\Phrase ();
				$subquery->setSlop ( $this->getSlop () );
				
				foreach ( $this->_terms as $termId => $term ) {
					$qualifiedTerm = new \Core\Search\Lucene\Index\Term ( $term->text, $fieldName );
	...
	public function optimize(\Core\Search\Lucene\InterfaceLucene $index) {
		...
		if (count ( $this->_terms ) == 1) {
			// It's one term query
			$optimizedQuery = new \Core\Search\Lucene\Search\Query\Term ( reset ( $this->_terms ) );
	 ...
	 *
	 * @param integer $num        	
	 * @param \Core\Search\Lucene\Search\Weight\Term $weight        	
	 */
	...
	public function createWeight(\Core\Search\Lucene\InterfaceLucene $reader) {
		$this->_weight = new \Core\Search\Lucene\Search\Weight\Phrase ( $this, $reader );
		return $this->_weight;
	}
	 ...
	 * @return float
	 */
	public function _sloppyPhraseFreq($docId,\Core\Search\Lucene\InterfaceLucene $reader) {
		$freq = 0;
