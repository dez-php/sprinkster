...
class QueryLexer extends \Core\Search\Lucene\FSM {
	/**
	...
	public function __construct() {
		...
		$lexemeModifierErrorAction = new \Core\Search\Lucene\FSMAction ( $this, 'lexModifierErrException' );
		$quoteWithinLexemeErrorAction = new \Core\Search\Lucene\FSMAction ( $this, 'quoteWithinLexemeErrException' );
		$wrongNumberErrorAction = new \Core\Search\Lucene\FSMAction ( $this, 'wrongNumberErrException' );
		 ...
		 * Actions
		 */
		$syntaxLexemeAction = new \Core\Search\Lucene\FSMAction ( $this, 'addQuerySyntaxLexeme' );
		$lexemeModifierAction = new \Core\Search\Lucene\FSMAction ( $this, 'addLexemeModifier' );
		$addLexemeAction = new \Core\Search\Lucene\FSMAction ( $this, 'addLexeme' );
		$addQuotedLexemeAction = new \Core\Search\Lucene\FSMAction ( $this, 'addQuotedLexeme' );
		$addNumberLexemeAction = new \Core\Search\Lucene\FSMAction ( $this, 'addNumberLexeme' );
		$addLexemeCharAction = new \Core\Search\Lucene\FSMAction ( $this, 'addLexemeChar' );
	...
	public function tokenize($inputString, $encoding) {
		...
		$strLength = mb_strlen ( $inputString, mb_detect_encoding ( $inputString ) );
		
		// Workaround for iconv_substr bug
		$inputString .= ' ';
		
		for($count = 0; $count < $strLength; $count ++) {
			$this->_queryString [$count] = mb_substr ( $inputString, $count, 1, mb_detect_encoding ( $inputString ) );
		}
		...
		if ($this->getState () != self::ST_WHITE_SPACE) {
			throw new \Core\Search\Lucene\Search\QueryParserException ( 'Unexpected end of query' );
		}
	...
	public function addQuerySyntaxLexeme() {
		...
		if (strpos ( self::QUERY_DOUBLECHARLEXEME_CHARS, $lexeme ) !== false) {
			...
			if ($this->_queryStringPosition == count ( $this->_queryString ) || $this->_queryString [$this->_queryStringPosition] != $lexeme) {
				throw new \Core\Search\Lucene\Search\QueryParserException ( 'Two chars lexeme expected. ' . $this->_positionMsg () );
			}
		...
		$token = new \Core\Search\Lucene\Search\QueryToken ( \Core\Search\Lucene\Search\QueryToken::TC_SYNTAX_ELEMENT, $lexeme, $this->_queryStringPosition );
		
		// Skip this lexeme if it's a field indicator ':' and treat previous as
		// 'field' instead of 'word'
		if ($token->type == \Core\Search\Lucene\Search\QueryToken::TT_FIELD_INDICATOR) {
			$token = array_pop ( $this->_lexemes );
			if ($token === null || $token->type != \Core\Search\Lucene\Search\QueryToken::TT_WORD) {
				throw new \Core\Search\Lucene\Search\QueryParserException ( 'Field mark \':\' must follow field name. ' . $this->_positionMsg () );
			}
			
			$token->type = \Core\Search\Lucene\Search\QueryToken::TT_FIELD;
		}
	...
	public function addLexemeModifier() {
		$this->_lexemes [] = new \Core\Search\Lucene\Search\QueryToken ( \Core\Search\Lucene\Search\QueryToken::TC_SYNTAX_ELEMENT, $this->_queryString [$this->_queryStringPosition], $this->_queryStringPosition );
	}
	...
	public function addLexeme() {
		$this->_lexemes [] = new \Core\Search\Lucene\Search\QueryToken ( \Core\Search\Lucene\Search\QueryToken::TC_WORD, $this->_currentLexeme, $this->_queryStringPosition - 1 );
	...
	public function addQuotedLexeme() {
		$this->_lexemes [] = new \Core\Search\Lucene\Search\QueryToken ( \Core\Search\Lucene\Search\QueryToken::TC_PHRASE, $this->_currentLexeme, $this->_queryStringPosition );
	...
	public function addNumberLexeme() {
		$this->_lexemes [] = new \Core\Search\Lucene\Search\QueryToken ( \Core\Search\Lucene\Search\QueryToken::TC_NUMBER, $this->_currentLexeme, $this->_queryStringPosition - 1 );
		$this->_currentLexeme = '';
	}
	...
	public function lexModifierErrException() {
		throw new \Core\Search\Lucene\Search\QueryParserException ( 'Lexeme modifier character can be followed only by number, white space or query syntax element. ' . $this->_positionMsg () );
	}
	public function quoteWithinLexemeErrException() {
		throw new \Core\Search\Lucene\Search\QueryParserException ( 'Quote within lexeme must be escaped by \'\\\' char. ' . $this->_positionMsg () );
	}
	public function wrongNumberErrException() {
		throw new \Core\Search\Lucene\Search\QueryParserException ( 'Wrong number syntax.' . $this->_positionMsg () );
	}
}
