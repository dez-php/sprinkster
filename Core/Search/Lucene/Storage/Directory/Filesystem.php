...
namespace Core\Search\Lucene\Storage\Directory;

class Filesystem extends \Core\Search\Lucene\Storage\Directory {
	/**
	...
	/**
	 * Cache for \Core\Search\Lucene\Storage\File\Filesystem objects
	 * Array: filename => \Core\Search\Lucene\Storage\File object
	 *
	 * @var array
	...
	public function __construct($path) {
		if (! is_dir ( $path )) {
			if (file_exists ( $path )) {
				...
				throw new \Core\Search\Lucene\Exception ( 'Path exists, but it\'s not a directory' );
			} else {
				if (! self::mkdirs ( $path )) {
					require_once 'Search/Lucene/Exception.php';
					throw new \Core\Search\Lucene\Exception ( "Can't create directory '$path'." );
				}
			}
		}
		$this->_dirPath = $path;
	...
	public function createFile($filename) {
		...
		$this->_fileHandlers [$filename] = new \Core\Search\Lucene\Storage\File\Filesystem ( $this->_dirPath . '/' . $filename, 'w+b' );
	...
	public function deleteFile($filename) {
		...
		if (! @unlink ( $this->_dirPath . '/' . $filename )) {
			...
			require_once 'Search/Lucene/Exception.php';
			throw new \Core\Search\Lucene\Exception ( 'Can\'t delete file: ' . $php_errormsg );
		}
		ini_set ( 'track_errors', $trackErrors );
	}
	...
	public function renameFile($from, $to) {
		...
		if (file_exists ( $this->_dirPath . '/' . $to )) {
			if (! unlink ( $this->_dirPath . '/' . $to )) {
				require_once 'Search/Lucene/Exception.php';
				throw new \Core\Search\Lucene\Exception ( 'Delete operation failed' );
			}
		}
		...
		if (! $success) {
			ini_set ( 'track_errors', $trackErrors );
			require_once 'Search/Lucene/Exception.php';
			throw new \Core\Search\Lucene\Exception ( $php_errormsg );
		}
	...
	public function getFileObject($filename, $shareHandler = true) {
		...
		if (! $shareHandler) {
			return new \Core\Search\Lucene\Storage\File\Filesystem ( $fullFilename );
		}
		...
		$this->_fileHandlers [$filename] = new \Core\Search\Lucene\Storage\File\Filesystem ( $fullFilename );
		return $this->_fileHandlers [$filename];
	}
}
