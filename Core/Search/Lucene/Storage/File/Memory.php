...
namespace Core\Search\Lucene\Storage\File;

class Memory extends \Core\Search\Lucene\Storage\File {
	/**
	 * FileData
	 *
	 * @var string
	...
	public function readLong() {
		...
		if (PHP_INT_SIZE > 4) {
			...
			if ((ord ( $str [0] ) != 0) || (ord ( $str [1] ) != 0) || (ord ( $str [2] ) != 0) || (ord ( $str [3] ) != 0) || ((ord ( $str [0] ) & 0x80) != 0)) {
				throw new \Core\Search\Lucene\Exception ( 'Largest supported segment size (for 32-bit mode) is 2Gb' );
			}
			
			return ord ( $str [4] ) << 24 | ord ( $str [5] ) << 16 | ord ( $str [6] ) << 8 | ord ( $str [7] );
		}
	}
	...
	public function writeLong($value) {
		...
		if (PHP_INT_SIZE > 4) {
			...
			$this->_data .= chr ( 0 ) . chr ( 0 ) . chr ( 0 ) . chr ( 0 ) . chr ( $value >> 24 & 0xFF ) . chr ( $value >> 16 & 0xFF ) . chr ( $value >> 8 & 0xFF ) . chr ( $value & 0xFF );
		}
	...
	public function writeString($str) {
		...
		if ($chars < 0) {
			throw new \Core\Search\Lucene\Exception ( 'Invalid UTF-8 string' );
		}
