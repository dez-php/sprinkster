...
class Db extends \Core\Base\Widget {
	...
	protected $referenceRowCallAuto = array('getUserFullname');
	...
	protected $helpText;
	protected $page_var = 'page';
	
	protected $edit_route = null;
	...
	public function preDispatch() {
		...
		foreach($_SESSION AS $key => $value) {
			if(preg_match('~(_form-|form-data|'.$this->id.'-data)~i', $key))
				unset($_SESSION[$key]);
		}
		...
		if($request->getQuery('order')) {
			$this->order = strtolower($request->getQuery('order')) == 'asc' ? 'ASC' : 'DESC';
		...
		//var_dump( $fnc() ); exit;
		$this->page = (int)$request->getQuery($this->page_var) > 0 ? (int)$request->getQuery($this->page_var) : 1;

		if( is_array($filters = $request->getQuery('filter')) ) {
			$makeFilter = array();
			foreach($filters AS $filter => $value) {
				if($filter == 'autocomplete') {
					foreach($value AS $table => $col) {
						try {
							$table = '\\' . ltrim($table,'\\');
							$table = new $table;
							if(method_exists($table, 'autocomplete_search')) {
								list($col, $val) = each($col);
								if($val && trim($val)) {
									$makeFilter[$col] = $table->autocomplete_search($val);
								}
							}
						} catch (\Core\Exception $e) {}
					}
				} elseif( in_array($filter, $this->info['cols']) && $value !== '' && $value !== '*' && !is_null($value) ) {
					$rowType = $this->info['metadata'][$filter]['DATA_TYPE'];
					if(in_array($rowType, array('text','varchar', 'char'))) {
						if($filter == 'email') {
							$makeFilter[$filter] = new \Core\Db\Expr($this->dataProvider->getAdapter()->quote('%' . $value . '%'));
						} else {
	...
	public function setColumns($columns = array()) {
		...
		foreach($columns AS $row => $column) {
			if(isset($column['virtual']) && is_array($column['virtual']) && $column['virtual']) {
			...
			} else if(is_array($column) && isset($column['name'])) {
				...
				$this->columns[$row] = array(
						'name' => $column['name'],
						'type' => isset($column['type'])&&$column['type']?$column['type']:(isset($this->info['metadata'][$column['name']]) ? $this->info['metadata'][$column['name']]['DATA_TYPE']: null),
						'value' => isset($column['value'])&&$column['value']?$column['value']:self::DEFAULT_VALUE_STORED,
						'label' => isset($column['label'])&&$column['label']?$column['label']:\Core\Camel::toCamelCase($column['name'], true, true),
						'default' => isset($column['default'])?$column['default']:(isset($this->info['metadata'][$column['name']]) ? $this->info['metadata'][$column['name']]['DEFAULT'] : null),
						'length' => isset($column['length'])?$column['length']:(isset($this->info['metadata'][$column['name']]) ? $this->info['metadata'][$column['name']]['LENGTH'] : null),
						'primary' => isset($column['primary'])?$column['primary']:(isset($this->info['metadata'][$column['name']]) ? $this->info['metadata'][$column['name']]['PRIMARY'] : null),
	...
	public function findColum(\Core\Db\Table\Row $data, $column = null) {
		...
		$search = null;
		foreach( $this->referenceRowCallAuto AS $name) {
			if(is_callable([$data, $name]) && method_exists($data, $name)) {
				return $data->$name();
			}
		}
	...
	public function result() {
		...
		$data['results'] = $this->dataProvider->fetchAll($this->sql && $this->sql instanceof \Core\Db\Select ? $this->sql : $this->filter, $order, $limit, $offset);

		if(!$this->nopagination && !$this->hidepagination) {
			///pagination
			$total_records = $this->dataProvider->countBy($this->sql && $this->sql instanceof \Core\Db\Select ? $this->sql : $this->filter);
			$data['page'] = $this->page;
			...
			$pagination->setUrl( $this->rebuldLink($request->getFullUrl(),array($this->page_var => '{page}')) );
