...
use Facebook\Library\Authentication\AccessToken;
use Facebook\Library\Authentication\OAuth2Client;
use Facebook\Library\FileUpload\FacebookFile;
use Facebook\Library\FileUpload\FacebookVideo;
use Facebook\Library\GraphNodes\GraphEdge;
use Facebook\Library\Url\UrlDetectionInterface;
use Facebook\Library\Url\FacebookUrlDetectionHandler;
use Facebook\Library\PseudoRandomString\PseudoRandomStringGeneratorInterface;
use Facebook\Library\PseudoRandomString\McryptPseudoRandomStringGenerator;
use Facebook\Library\PseudoRandomString\OpenSslPseudoRandomStringGenerator;
use Facebook\Library\PseudoRandomString\UrandomPseudoRandomStringGenerator;
use Facebook\Library\HttpClients\FacebookHttpClientInterface;
use Facebook\Library\HttpClients\FacebookCurlHttpClient;
use Facebook\Library\HttpClients\FacebookStreamHttpClient;
use Facebook\Library\HttpClients\FacebookGuzzleHttpClient;
use Facebook\Library\PersistentData\PersistentDataInterface;
use Facebook\Library\PersistentData\FacebookSessionPersistentDataHandler;
use Facebook\Library\PersistentData\FacebookMemoryPersistentDataHandler;
use Facebook\Library\Helpers\FacebookCanvasHelper;
use Facebook\Library\Helpers\FacebookJavaScriptHelper;
use Facebook\Library\Helpers\FacebookPageTabHelper;
use Facebook\Library\Helpers\FacebookRedirectLoginHelper;
use Facebook\Library\Exceptions\FacebookSDKException;
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['http_client_handler'])) {
            if ($config['http_client_handler'] instanceof FacebookHttpClientInterface) {
                ...
                throw new \InvalidArgumentException('The http_client_handler must be set to "curl", "stream", "guzzle", or be an instance of Facebook\Library\HttpClients\FacebookHttpClientInterface');
            }
        }
        ...
        if (isset($config['url_detection_handler'])) {
            if ($config['url_detection_handler'] instanceof UrlDetectionInterface) {
                ...
                throw new \InvalidArgumentException('The url_detection_handler must be an instance of Facebook\Library\Url\UrlDetectionInterface');
            }
        }
        ...
        if (isset($config['pseudo_random_string_generator'])) {
            if ($config['pseudo_random_string_generator'] instanceof PseudoRandomStringGeneratorInterface) {
                ...
                throw new \InvalidArgumentException('The pseudo_random_string_generator must be set to "mcrypt", "openssl", or "urandom", or be an instance of Facebook\Library\PseudoRandomString\PseudoRandomStringGeneratorInterface');
            }
        }
        ...
        if (isset($config['persistent_data_handler'])) {
            if ($config['persistent_data_handler'] instanceof PersistentDataInterface) {
                ...
                throw new \InvalidArgumentException('The persistent_data_handler must be set to "session", "memory", or be an instance of Facebook\Library\PersistentData\PersistentDataInterface');
            }
        }
