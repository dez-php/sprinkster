...
class Elfinder {
	...
	{ 
		...
		if($info) {
			$image = new \Local\Library\Thumb($target);
			$image->rotate(90);
			$image->save($target);
		}
	...
	protected function _checkName($n)
	{
		...
		return preg_match('|^[^\\/\<\>:]+$|', $n) ? preg_replace('~([ ]{1,})~', '-', $n) : false;
	}
	...
	protected function _mimetype($path)
	{
		...
		switch ($this->_options['mimeDetect']) {
			...
			case 'bsd':   
				...
				break;
			case 'jo':   
				$type = \Core\File\Ext::getMimeFromFile( basename($path) );
				break;
			default:
				$pinfo = pathinfo($path); 
	...
	protected function _tmb($img, $tmb)
	{
		...
		switch ($this->_options['imgLib']) {
			...
			case 'gd':
				...
				$img = new \Local\Library\Thumb($img);
				if ($this->_options['tmbCrop'] == false) {
	...
	protected function _resizeImg($img, $w, $h)
	{
		...
		switch ($this->_options['imgLib']) {
			...
			case 'gd':
				...
				$res = new \Local\Library\Thumb($img);
				$res->resize($w, $h);
				return $res->save($img);
