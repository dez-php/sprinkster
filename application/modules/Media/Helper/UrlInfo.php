...
namespace Media\Helper;

class UrlInfo {
	const AE_TAG = '<!-- Generated by UrlInfo -->';
	private $_media_id;
	private $_stub;
	private $_object_attribs;
	private $_object_params;
	private $media_stubs = null;
	
	public function __construct() { 
		$this->getStumbs();
	}
	
	private function getStumbs() {
		static $media_stubs = null;
		if($media_stubs === null) {
			$media_stubs = array();
			//set config's data Recursive
			if(is_dir(__DIR__ . '/../media/')) {
				$Directory = new \RecursiveDirectoryIterator(__DIR__ . '/../media/');
				$Iterator = new \RecursiveIteratorIterator($Directory);
				$objects  = new \RegexIterator($Iterator, '/^.+\.php$/i');
				foreach($objects as $name => $object){
					try {
						$options = include_once $name;
						if(isset($options['media_stubs']) && is_array($options['media_stubs']) && $options['media_stubs']) {
							foreach($options['media_stubs'] AS $key => $option) {
								$media_stubs[$key] = $option;
							}
						}
					} catch (\Exception $e) {}
				}
		...
		return $this->media_stubs = $media_stubs;
	}
	 ...
	 *
	 * @param $url string
	 *        	- href to check for embeded video
	 *        	
	 * @return boolean - whether or not the url contains valid/supported video
	 */
	public function parseUrl($url) {
		if(is_array($this->media_stubs)) {
			foreach ( $this->media_stubs as $stub ) {
	...
	public function getStub($property = null) {
		return isset ( $this->_stub [$property] ) ? $this->_stub [$property] : $this->_stub;
	}
	...
	public function getImageURL() {
		...
		$thumb = preg_replace_callback('~(\{([^\}]{1,})\})~', function($m) {
			$res=$m[1];
			try { $fnc = create_function('','return '.$m[2].';'); $res = $fnc(); } catch (\Exception $e) { }
			return $res;
		}, $thumb);
	 ...
	 *
	 * @param
	 *        	mixed - height to set the object to
	 *        	
	 ...
	 *
	 * @param
	 *        	mixed - width to set the object to
	 *        	
	 * @return boolean - true if the value was set, false
	 *         if parseURL hasn't been called yet
	 */
	 ...
	 *
	 * @param $param mixed
	 *        	- the name of the param to be set
	 *        	or an array of multiple params to set
	 * @param $value string
	 *        	- (optional) the value to set the param to
	...
	private function _buildiFrame() {
		...
		/*
		 * if(strpos($source, '?')) { $source .=
		 * '&autohide=1&theme=light&hd=1&modestbranding=1&rel=0&showinfo=0&showsearch=0&wmode=transparent&autoplay=1';
		 * } else { $source .=
		 * '?autohide=1&theme=light&hd=1&modestbranding=1&rel=0&showinfo=0&showsearch=0&wmode=transparent&autoplay=1';
		 * }
		 */
	...
	private function _setDefaultParams() {
		...
		$flashvars = htmlspecialchars ( $flashvars, ENT_QUOTES, 'utf-8', false );
