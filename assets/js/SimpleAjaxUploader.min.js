 ...
 * Extended by jooorooo@gmail.com
 */
  ...
  */
  _createInput: function() {
    ...
    ss.addEvent( this._input, 'mouseover', function() {
      ...
      self._opts.onMouseover.call(this,self);
    });
    ...
    ss.addEvent( this._input, 'mouseout', function() {
      ...
      self._opts.onMouseout.call(this,self);
    });

    ss.addEvent( this._input, 'focus', function() {
      ss.addClass( self._overBtn, self._opts.focusClass );
      self._opts.onFocus.call(this,self);
    });

    ss.addEvent( this._input, 'blur', function() {
      ss.removeClass( self._overBtn, self._opts.focusClass );
      self._opts.onBlur.call(this,self);
    });
...
//    	return;
      try {
    	  ...
    	  response = ss.parseJSON( $('<textarea/>').val(response).val() );
	  }
      if ( response === false ) {
  ...
  */
  _uploadXhr: function( filename, size, sizeBox, progBar, progBox, pctBox ) {
    ...
    // Build query string
    queryURL = settings.url + (settings.url.indexOf('?') > -1 ? '&' : '?') + ss.obj2string( params );
  ...
  */
  _uploadIframe: function( filename, sizeBox, progBar, progBox, pctBox ) {
    ...
    if ( this._opts.nginxProgressUrl ) {
      form.action = this._opts.url + (this._opts.url.indexOf('?') > -1 ? '&' : '?') + this._opts.nginxProgressHeader + '=' + key;
    }
    ...
    callback = ss.addEvent( iframe, 'load', function() {
      try {
        var doc = iframe.contentDocument ?
            ...
            response = (doc.body.textContent || doc.body.innerHtml);
  ...
  */
  _getProg: function( key, progBar, sizeBox, pctBox, counter ) {
    ...
    if ( this._opts.nginxProgressUrl ) {
      url = self._opts.nginxProgressUrl + (self._opts.nginxProgressUrl.indexOf('?') > -1 ? '&_=' : '?_=') + time;

    // PHP APC upload progress
    } else if ( this._opts.progressUrl ) {
      url = self._opts.progressUrl + (self._opts.progressUrl.indexOf('?') > -1 ? '&' : '?') + 'progresskey=' + encodeURIComponent( key ) + '&_=' + time;
    }
